...
$res = $rpip-> reporteImprimirProductosInocuidadPlaguicida($conexion);
	...
	echo '<tr>';
		...
		echo'<td>';
			foreach ((array)$registros['usos'] as $usos) {																																																																																		  
			    $usoss1.= $usos['nombre_uso'].' - '.$usos['nombre_comun'].' - '.$usos['instalacion'].', ';
			}
			echo rtrim ($usoss1, ' ,' );
		echo '</td>';
		...
		echo'<td>';
			foreach ((array)$registros['formulador'] as $formulador) { 
			    $formuladores.=$formulador['formulador'].', ';	
			}
