...
try {
	try {
		...
		$identificadorResponsableRegistro = $_SESSION['usuario'];
		
		$codigoAccionPersonal = '';
		
		$filaSolicitud = pg_fetch_assoc($cc->obtenerDatosPermiso($conexion,$id_registro));
		$id_area_permiso = $filaSolicitud['id_area_permiso'];
		...
		//Obtiene información de jefe superior PC Dirección Ejecutiva Prov Director Tipo A
        if($id_area_permiso == 'DGATH'){
            $aprobador = pg_fetch_assoc($cc->obtenerDirectorXArea($conexion, 'DE'));
            $nombreDirector = $aprobador['nombreresponsable'];
            $cargoDirector = $aprobador['puestoresponsable'];
            
            //Director de Talento Humano
            $aprobadorTH = pg_fetch_assoc($cc->obtenerDirectorXArea($conexion, 'DGATH'));
            $nombreResponsableTH = $aprobadorTH['nombreresponsable'];
            $cargoResponsableTH = $aprobadorTH['puestoresponsable'];
        }else {
            $aprobador = pg_fetch_assoc($cc->obtenerDirectorXArea($conexion, $id_area_permiso));
            $nombreDirector = $aprobador['nombreresponsable'];
            $cargoDirector = $aprobador['puestoresponsable'];
            
            //Responsable distrital TTHH
            $aprobadorTH = pg_fetch_assoc($cc->obtenerResponsableTTHHXArea($conexion, $id_area_permiso));
            $nombreResponsableTH = $aprobadorTH['nombreresponsable'];
            $cargoResponsableTH = $aprobadorTH['puestoresponsable'];
        }
		...
		*/
		...
		//Obtiene información de responsable de registro de acción de personal
		$filaResponsable = pg_fetch_assoc($cc->obtenerNombreDirector($conexion,$identificadorResponsableRegistro));
		$nombreResponsableRegistro = $filaResponsable['directorath'];
		$cargoResponsableRegistro = $filaResponsable['puestoth'];
		...
		$parameters['parametrosReporte'] = array(
			...
			'resolucion'=> ' ',
		    'responsableRegistro'=> $nombreResponsableRegistro,
		    'cargoResponsableRegistro'=> 'Responsable del registro',
		    'formaRegistro'=> 'Registrado mediante Sistema GUIA'
		);
