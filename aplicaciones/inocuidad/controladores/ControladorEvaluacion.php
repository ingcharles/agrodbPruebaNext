...
class ControladorEvaluacion
{
    ...
    public function getResultadoDatos($ic_analisis_muestra_id){
        ...
        $resultado = "";
        if (is_array($evaluaciones) && count($evaluaciones) >0){
            foreach ($evaluaciones as $evaluacion) {
                $res = "OK";
                $color="transparent";
                if($evaluacion['residuo']<$evaluacion['limite_minimo']){
                    $res="INFERIOR";
                    $color="#e8daaf";
                }else if($evaluacion['residuo']>$evaluacion['limite_maximo']){
                    $res="SUPERIOR";
                    $color="#e8afb6";
                }
                ...
                $resultado .= "<td style='text-align: right'>" . $evaluacion['residuo'] . "</td>";
