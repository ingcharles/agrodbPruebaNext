...
private $servicios;
    ...
    public function obtenerComboCatalogosOpcionesParam($tipoCatalogo,$param){
        ...
        switch ($tipoCatalogo){
            case 'PROGRAMAS':
                ...
                break;
            case 'PARAMETROS':
                $filas=$catagosServ->obtenerParametros($this->conexion);
                while($filasPrd=pg_fetch_assoc($filas)){
                    $dataCatalogos[]='<option value="'.$filasPrd['ic_catalogo_id']. '" data-grupo="'. $filasPrd['ic_catalogo_id'] . '">'. $filasPrd['nombre'] .'</option>';
                }
                unset($filas);
            ...
            case 'ORIGEN_MUESTRA':
                ...
                unset($filas);
                break;
            case 'ORIGEN_MUESTRA_TIPO':
                $filas=$catagosServ->obtenerCatalogoPorGrupoTipo($this->conexion,"origen_informacion",$param);
                while($filasPrd=pg_fetch_assoc($filas)){
                    $dataCatalogos[]='<option value="'.$filasPrd['ic_catalogo_id']. '" data-value="'. $filasPrd['valor']. '" data-grupo="'. $filasPrd['grupo'] . '">'. $filasPrd['nombre'] .'</option>';
                }
                unset($filas);
            ...
            case 'REQUERIMIENTOS':
                ...
                if ($param =='REPORTE'){
                    ...
                    $resPerfil = pg_fetch_assoc($this->servicios->obtenerPerfilUsuarioCaso($this->conexion,$identificador));
                ...
                $filas=$catagosServ->obtenerTipoRequerimiento($this->conexion,$banderaPerfil);
                while($filasPrd=pg_fetch_assoc($filas)){
            ...
            case 'RESULTADO_DESICION':
                ...
                break;
            case 'DATOS_TECNICO':
                $filas=$catagosServ->obtenerDatosTecnico($this->conexion,$param);
                while($filasPrd=pg_fetch_assoc($filas)){
                    $dataCatalogos=$filasPrd['nombre_completo'];
                }
                unset($filas);
                break;
        }
    ...
    public function obtenerComboCatalogosOpcionesPerfil($tipoCatalogo,$accion){
        return $this -> obtenerComboCatalogosOpcionesParam($tipoCatalogo,$accion);
    }
    ...
    public function obtenerNombreProgramaParametro($parametro_id){
        $catagosServ=new CatalogosDAO();
        $result="";
        if($parametro_id != ""){
            $programas=$catagosServ->obtenerParametroById($parametro_id,$this->conexion);
            $fila = pg_fetch_assoc($programas);
            $result=$fila['nombre'];
    ...
    public function listaSitiosFincas($provincia,$canton,$parroquia,$opcion){
        $dataCatalogos=[];
        $catagosServ=new CatalogosDAO();
        $filas=$catagosServ->obtenerSitiosParametros($provincia,$canton,$parroquia,$opcion,$this->conexion);
        while($filasPrd=pg_fetch_assoc($filas)){
    ...
    public function obtenerNumeroPorCertificado($certificado){
        $catagosServ=new CatalogosDAO();
        $filas=$catagosServ->obtenerNumeroPorCertificado($certificado,$this->conexion);
        $filasPrd=pg_fetch_assoc($filas);
        unset($filas);
        return $filasPrd;
    }
    ...
    public function cancelarRegistro($ic_requerimiento_id, $mensaje,$archivo,$usuario){
        $casoService = new ServiceCasoDAO();
        return $casoService->cancelarRegistro($ic_requerimiento_id,$mensaje,$archivo,$usuario, $this->conexion);
    }
