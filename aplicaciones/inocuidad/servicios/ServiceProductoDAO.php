...
class ServiceProductoDAO
{
    ...
    public function getInsumosMuestraRapida($ic_producto_id,$conexion){
        $queryAll="SELECT ic_producto_muestra_rapida_id,ic_producto_id, ic_insumo_id,um,limite_minimo,limite_maximo,ic_lmr_id
        ...
        try{
            $result = $conexion->ejecutarConsulta($queryAll);
            while ($filasProducto = pg_fetch_assoc($result)) {
                $producto = new ProductoMuestraRapida($filasProducto['ic_producto_muestra_rapida_id'],$filasProducto['ic_producto_id'],$filasProducto['ic_insumo_id'],$filasProducto['um'],$filasProducto['limite_minimo'],$filasProducto['limite_maximo'],$filasProducto['ic_lmr_id']);
                array_push($filas, $producto);
            }
        }catch(Exception $exc){
            return array();
    ...
    public function saveAndUpdateProducto(Producto $producto,$conexion){
        ...
        if(isset($producto)) {
            ...
            if ($producto->getIcProductoId() != null) {
                ...
                $querySave = "select * from g_inocuidad.ic_producto where producto_id = ".$producto_id." and nombre = '".$nombre."' and muestra_rapida = '".$muestra_rapida."'";
        ...
        if(isset($muestras)) {
            ...
            foreach ($muestras as $muestra){
                ...
                if($muestra->getIcProductoMuestraRapidaId() != null){
                    ...
                    $querySave .= "   SET ic_insumo_id=$ic_insumo_id, um='$um', limite_minimo=$limite_minimo, limite_maximo=$limite_maximo, ic_lmr_id=$ic_lmr_id";
                    ...
                    $querySave .= " um, limite_minimo, limite_maximo, ic_lmr_id)";
