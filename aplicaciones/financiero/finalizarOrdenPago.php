...
try{
	...
	if($ordenPago['tipo_solicitud'] != 'recargaSaldo'){  
		...
		$cc->actualizarTipoProcesoOrdenPago($conexion, $datos['id_pago'], 'factura');
		    ...
		    //echo "comprobanteX";		    
    		    ...
    		    if($ordenPago['estado_sri'] == ''){
    	            ...
    	            for ($k = 0; $k < count ($valorDepositado); $k++) {
						...
						switch ($formaPago[$k]){ 
							case 'Deposito':
								$cc -> guardarPagoOrden($conexion, $datos['id_pago'], $fechaDeposito[$k],$idBanco[$k],$nombreBanco[$k],$papeletaBanco[$k],$valorDepositado[$k],0,$idCuentaBanco[$k],$numeroCuentaBanco[$k]);
								break;
							case 'Efectivo':
								$cc -> guardarPagoOrden($conexion, $datos['id_pago'], $fechaDeposito[$k],0,'Pago en efectivo','Efectivo',$valorDepositado[$k]);
								break;
						}
