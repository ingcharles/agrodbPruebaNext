			...
			if($valorTotal != '0' && ($tipoSolicitud=="FitosanitarioExportacion" || $tipoSolicitud=="Importación" || $tipoSolicitud == "Fitosanitario")){
				...
				$ce->actualizarDatosSolicitudEtiqueta($conexion,'estado',$estadoEtiqueta,$idSolicitud);
			     ...
			     if($valorTotal == '0'){
			      ...
			      $cdp -> actualizarEstadoSolicitud($conexion, $estado, $idSolicitud, $mensaje);
			    ...
			    $cee->actualizarEstadoSolicitudPorIdSolicitud($conexion, $idSolicitud, $estado);
				...
				$qDatosProvincia = $ccf->obtenerDatosProvinciaPorIdCertificadoFitosanitario($conexion, $idSolicitud);
				...
				$qDatosProvincia = $cmp->obtenerDatosSolicitudProvincia($conexion, $idSolicitud);
			    $datosProvincia = pg_fetch_assoc($qDatosProvincia);
			    $provinciaSolicitud = $datosProvincia['provincia'];
				
			}else if ($tipoSolicitud=="registroProductoRia"){
                ...
                $crp->actualizarEstadoSolicitudPorIdSolicitud($conexion, $idSolicitud, $estado);
				
				$qDatosProvincia = $crp->abrirSolicitudPorNumeroSolicitud($conexion, $idSolicitud);
                $datosProvincia = pg_fetch_assoc($qDatosProvincia);
