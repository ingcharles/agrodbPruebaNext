...
$itemsFiltrados[] = array();
	...
	switch ($tipoSolicitud){
		...
		case 'ensayoEficacia':
		    ...
		    if( $estado=='pago'){
		        $res=$cee->obtenerSolicitudPorEstadoProvincia($conexion, $estado, $provinciaUsuario);
		    }else if( $estado=='verificacion'){
		        $res=$cee->obtenerSolicitudPorEstadoProvincia($conexion, $estado, $provinciaUsuario);
		    }else if( $estado=='verificacionVUE'){
		        $res=$cee->obtenerSolicitudPorEstadoProvincia($conexion, 'verificacion', $provinciaUsuario);
		    }
        ...
        if ($estado == 'pago') {
            $res = $crp->obtenerSolicitudPorEstadoProvincia($conexion, $estado, $provinciaUsuario);
        } else if ($estado == 'verificacion') {
            $res = $crp->obtenerSolicitudPorEstadoProvincia($conexion, $estado, $provinciaUsuario);
        } else if ($estado == 'verificacionVUE') {
            $res = $crp->obtenerSolicitudPorEstadoProvincia($conexion, 'verificacion', $provinciaUsuario);
        }

        $nombreArchivo = 'abrirSolicitudEnviadaFinanciero';
        $nombreModulo = 'registroProductoRia';
        $nombreSolicitud = $tipoSolicitud;
        ...
        if($estado == 'pago'){
            $res = $cim -> listarImportacionMuestrasRevisionFinancieroRS($conexion, $provinciaUsuario, $estado);
        }else if ($estado == 'verificacionVUE'){
            $res = $cim -> obtenerImportacionMuestrasFinancieroVerificacion($conexion, $estado, $provinciaUsuario, $tipoSolicitud);
        }else if ($estado == 'verificacion'){            
            $res = $cim -> listarImportacionMuestrasRevisionFinancieroRS($conexion, $provinciaUsuario, $estado);
        }
        
        
        $nombreArchivo = 'abrirImportacionMuestrasEnviadaFinanciero';
        $nombreModulo = 'importacionMuestras';
	...
	if($tipoSolicitud=='Emisión de Etiquetas' || $tipoSolicitud == 'dossierPecuario' || $tipoSolicitud == 'dossierPlaguicida' || 
		...
		$tipoSolicitud == 'certificadoFito' || $tipoSolicitud == 'modificacionProductoRia' || $tipoSolicitud == 'registroProductoRia'){
