		...
		<fieldset>
			<legend>Resultado de la Inspección</legend>			
    			...
    			<div data-linea="28" class="auditoriaInspeccion">
					<label>Fecha Auditoría Programada: </label>
					<input type="text" id="fechaAuditoriaProgramada" name="fechaAuditoriaProgramada" required="required" readonly="readonly" 
						value="<?php echo $filaSolicitud['fecha_auditoria_programada']; ?>" />
				</div>
...
<script type="text/javascript">
	...
	$("#fechaAuditoriaRealizada").datepicker({ 
	 ...
	 $("#fechaAuditoriaProgramada").datepicker({ 
		changeMonth: true,
		changeYear: true,
		dateFormat: 'yy-mm-dd'
	 });
    ...
    function chequearCamposInspeccionTecnico(form){
		...
		if(!$.trim($("#fechaAuditoriaProgramada").val())){
			error = true;
			$("#fechaAuditoriaProgramada").addClass("alertaCombo");
		}
