				...
				case 'operadoresPEC' :
					...
					case 'transitoInternacional' :
					    ...
					    for ($i = 0; $i < count ($operacion); $i++) {
					        ...
					        echo '<fieldset>
										<legend>Solicitud N° </label>' .$solicitud['id_transito_internacional'].'</legend>
						...
						case 'ImportacionMuestrasVUE' :
					    $cim = new ControladorImportacionMuestras();
					    
					    $usuarioPorArea = false;
					    
					    for ($i = 0; $i < count ($operacion); $i++) {
					        $solicitud = pg_fetch_assoc($cim->abrirImportacionMuestras($conexion, $operacion[$i]));
					        
					        echo '<fieldset>
										<legend>Solicitud N° </label>' .$solicitud['id_importacion_muestras'].'</legend>
											    
										<div data-linea="1">
											<label>Solicitud VUE: </label>'.$solicitud['req_no']. '<br/>
										</div>
                                        <div data-linea="2">
											<label>Tipo solicitud: </label>'.$solicitud['nombre_tipo_solicitud']. '<br/>
										</div>
                                        <div data-linea="3">
											<label>Identificador operador: </label>'.$solicitud['identificador_importador']. '<br/>
										</div>
										<div data-linea="4">
											<label>Razón social: </label>'.$solicitud['razon_social_importador']. '<br/>
										</div>
										<div data-linea="5">
											<label>País origen: </label>'.$solicitud['nombre_pais_origen']. '<br/>
										</div>
										<div data-linea="6">
											<label>Estado: </label>'.$solicitud['estado']. '<br/>
										</div>
					        ...
					        $res=$cim->abrirImportacionMuestrasProductos($conexion, $operacion[$i]);
					        $fila=null;
					        ...
					        while($fila=pg_fetch_assoc($res)){
					            $contador+=1;
					            echo '<fieldset id="datosProducto">	<legend>Datos del Producto '.$contador.' :</legend>'.
									            '<div data-linea="1"><label>Nombre del producto: </label>'.$fila['nombre_producto'].'</div>'.
									            '<div data-linea="2"><label>Subpartida arancelaria: </label>'.$fila['subpartida'].'</div>'.
									            '<div data-linea="3"><label>Cantidad de producto: </label>'.$fila['cantidad_producto'].' '.$fila['unidad_medida_producto'].'</div>'.
									            '<div data-linea="3"><label>Peso: </label>'.$fila['peso'].' '.$fila['unidad_peso'].'</div>'.
									            '</fieldset>';
					        }
					    }
					    ...
					    switch($solicitud['codigo_tipo_solicitud']){
					        case 'SA':
					            $perfil = 'PFL_INS_IMMU_SA';
					        break;
					        case 'SV':
					            $perfil = 'PFL_INS_IMMU_SV';
					        break;
					        case 'IAV':
					        case 'IAPB':
					        case 'IAPQ':
					        case 'IAF':
					            $perfil = 'PFL_INS_IMMU_RIA';
					        break;
					        default:
					            $perfil = '';
					        break;
