...
try{
	...
	$arrayResultados = array('noHabilitado','subsanacion','subsanacionRepresentanteTecnico','subsanacionProducto', 'declararProveedor');//Verificar si existe subsanacion
	...
	try {
		...
		switch ($tipoSolicitud){
			...
			case 'ImportacionMuestrasVUE' :
			    ...
			    $cim->enviarImportacionMuestras($conexion, $idSolicitud, $resultadoDocumento);
			    
			    if($resultadoDocumento =='subsanacion' && $idVue!=''){
			        $cVUE->ingresarSolicitudesXatenderGUIA('101-062-REQ','410','21',$idVue, 'Por atender', $observacionesDocumento);
			    }
			    
			    if($resultadoDocumento =='rechazado' && $idVue!=''){
			        $cVUE->ingresarSolicitudesXatenderGUIA('101-062-REQ','310','21',$idVue, 'Por atender', $observacionesDocumento);
			    }
			    
			    if($resultadoDocumento == 'aprobado'){
			        
			        $area = pg_fetch_assoc($cim->abrirImportacionMuestras($conexion, $idSolicitud));
			        $cim->enviarFechaVigenciaImportacionMuestras($conexion, $idSolicitud, $area['codigo_tipo_solicitud']);
			        $cim->cambiarEstadoImportacionMuestras($conexion, $idSolicitud, $resultadoDocumento, $inspector, $observacionesDocumento);
			        
			        $cVUE->ingresarSolicitudesXatenderGUIA('101-062-REQ','320','21',$idVue, 'Por atender');
			    }
