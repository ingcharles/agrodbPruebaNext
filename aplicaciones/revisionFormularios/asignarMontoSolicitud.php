	...
	try {
		...
		switch ($tipoSolicitud){
			...
			case 'dossierPecuario':
			    ...
			    $cdpmvc->actualizarEstadoSolicitud($conexion, 'verificacion', $idSolicitud, 'Proceso de Asignación de Tasa');
			...
			case 'ImportacionMuestrasVUE' :
			    ...
			    if($monto == '0'){
			        ...
			        //Asigna el resultado de revisión de pago de solicitud de importacion
			        $cim->enviarImportacionMuestras($conexion, $idSolicitud, 'aprobado');
			        ...
			        if($envioVue != 'tarifarioAntiguo'){
			            $cim->enviarImportacionMuestras($conexion, $idSolicitud, $estado);
			            if($idVue != '' ){
			                $cVUE->ingresarSolicitudesXatenderGUIA('101-062-REQ','120','21',$idVue, 'Por atender');
			            }
			        }
			    }
