		...
		switch ($tipoSolicitud){
				...
				if(count($idAreas)>0){
					...
					if($resultado == 'registrado'){
						foreach ($idGrupoSolicitudes as $solicitud){
							...
							if($operacion['proceso_modificacion'] == 't' && ($opcionArea == 'MDT' || $opcionArea == 'MDC')){
								$actualizacionFechas =true;
							}
							...
							switch ($estado['estado']){
								...
								case 'cargarProducto':
									...
									if($generarReporteEmpresas){
										...
										$rutaArchivoCodigoQr = 'http://181.112.155.163/'.$constg::RUTA_APLICACION.$salidaReporte;
											...
											case 'AI':
												...
												switch ($opcionArea){
													...
													case 'ACO':
														...
														$cr->actualizarCentrosAcopioInspeccion($conexion, $idOperadorTipoOperacion, 'sistemaGUIA', 'generado');
													...
													case 'MDT':	
														...
														$cr->actualizarDatosVehiculoInspeccion($conexion, $idOperadorTipoOperacion, 'sistemaGUIA', 'generado');
			...
			case 'certificacionBPA':
				...
				if($_POST['tipoInspector'] == 'Técnico'){
					...
					//Actualizar auditoria programada
				    $ccb->actualizarAuditoriaProgramada($conexion, $idSolicitud, $_POST['fechaAuditoriaProgramada']);
