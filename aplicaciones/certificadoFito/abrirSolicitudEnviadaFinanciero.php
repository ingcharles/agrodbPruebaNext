...
<header>
	...
	<fieldset>
		...
		<div data-linea="10">
			<label>Observación técnica: </label> <?php echo $filaSolicitud['observacion_revision'] = '' ? 'N/A' : $filaSolicitud['observacion_revision']; ?>
		</div>
			...
			<table id="tbExportadoresProductos" style="width: 100%">
				<thead>
					<tr>
						...
						<th style="width: 15%;">Nombre Subtipo Producto</th>
				...
				while ($fila = pg_fetch_assoc($res)){
				    ...
				    $qCantidadComercial = pg_fetch_assoc($cca->obtenerUnidadesFitosanitarias($conexion, $fila['id_unidad_cantidad_comercial']));
				    $codigoCantidadComercial = $qCantidadComercial['codigo_unidad_fitosanitaria'];
				    
				    if(isset($fila['id_unidad_peso_bruto'])){
				        $qPesoBruto = pg_fetch_assoc($cca->obtenerUnidadesFitosanitarias($conexion, $fila['id_unidad_peso_bruto']));
				        $codigoPesoBruto = $qPesoBruto['codigo_unidad_fitosanitaria'];
				    }
				    
				    $qPesoNeto = pg_fetch_assoc($cca->obtenerUnidadesFitosanitarias($conexion, $fila['id_unidad_peso_neto']));
				    $codigoPesoNeto = $qPesoNeto['codigo_unidad_fitosanitaria'];
