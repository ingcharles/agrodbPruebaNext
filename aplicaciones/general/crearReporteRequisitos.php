...
class PDF extends FPDF
{
	...
	public function Header(){
		...
		$img_file = $cts::RUTA_SERVIDOR_OPT . '/' . $cts::RUTA_APLICACION."/aplicaciones/general/img/fondoCertificadoBlanco.png";
	...
	function Body($idSolicitud, $tipo){
		...
		if($tipo == 'ImportacionMuestras'){
		    $cim = new ControladorImportacionMuestras();
		    $qRegistro = $cim->abrirImportacionMuestrasReporte($conexion, $idSolicitud);
		    $tipoRequisito = 'Exportación';
		    $ruta = 'destino';
		    
		    $this->Cell(0,30,utf8_decode('PERMISO DE IMPORTACIÓN DE MUESTRAS'),0,0);
		    $this->Ln(5);
		    $this->Cell(60);
		    //$this->Cell(0,10,utf8_decode('IMPORTATION PHITOSANITARY PERMIT'),0,0);
		}
		...
		if($tipo == 'Fitosanitario'){
		...
		}else if ($tipo == 'ImportacionMuestras'){
		    $seccion = true;
		    $tipoProcesados = false;
		    ...
		    foreach ($qRegistro as $registro){
		        ...
		        if ($seccion){
		            ...
		            $this->Cell(0,6,utf8_decode('3. DATOS DEL EXPORTADOR'),0,1,'L',true);
		            ...
		            $this->Cell(0,6,utf8_decode('4. INFORMACIÓN DE MUESTRAS'),0,1,'L',true);
		            
		            $this->SetFont('Times','',10);
		            $this->MultiCell(0,5,utf8_decode('País Origen: '.$registro['paisOrigen']),0,1);
		            $this->MultiCell(0,5,utf8_decode('País Embarque: '.$registro['paisEmbarque']),0,1);
		            $this->MultiCell(0,5,utf8_decode('País Destino: '.$registro['paisDestino']),0,1);
		            $this->MultiCell(0,5,utf8_decode('Puerto de Embarque: '.$registro['puertoEmbarque']),0,1);
		            $this->MultiCell(0,5,utf8_decode('Puerto de Destino: '.$registro['puertoDestino']),0,1);
		            ...
		            $this->Cell(0,6,utf8_decode('5. LISTA DE PRODUCTOS DE MUESTRA'),0,1,'L',true);
		        ...
		        $this->SetFont('Times','',10);
		        //Producto
		        $this->Cell(0,6,utf8_decode('Partida Arancelaria: '.$registro['subpartidaArancelaria']),0,1,'L',true);
		        
		        //Producto
		        $this->Cell(0,6,utf8_decode('Nombre del Producto: '.$registro['nombreProducto']),0,1,'L',true);
		        
		        //Cantidad
		        $this->Cell(0,6,utf8_decode('Cantidad/Unidad de Medida: '.$registro['cantidadProducto']. ' ' . $registro['unidadMedidaProducto']),0,1,'L',true);
		        // Salto de línea
		        
		        // Peso neto
		        $this->Cell(0,6,utf8_decode('Peso Neto del producto: '.$registro['peso']. ' ' . $registro['unidadPeso']),0,1,'L',true);
		        // Salto de línea
	...
	function Detalle($idSolicitud, $tipo){
		...
		if($tipo == 'ImportacionMuestras'){
		    ...
		    $this->Cell(0,10,utf8_decode('PERMISO DE IMPORTACIÓN DE MUESTRAS'),0,0);
		    $this->Ln(5);
		    $this->Cell(60);
		    //$this->Cell(0,10,utf8_decode('IMPORTATION FITOSANITARY PERMIT'),0,0);
		}
