		...
		<legend>Centros de faenamiento agregados</legend>		
			...
			if(pg_num_rows($consulta) > 0){
				while ($fila = pg_fetch_assoc($consulta)) {
					...
					$contenidoTablaCentros .=  '<tr id='.$fila['id_centros_faenamiento_transporte'].'><td>'.++$contadorProducto.'</td>
 ...
 */
 ...
 $("#registroContenedorVehiculo").change(function() {
	...
	function limpiarCamposVehiculo(){
		...
		$('#clase').val('');
		$('#servicio').val('');
		$('#capacidadInstalada').val('');
		$('#unidadMedida').val('');
		$('#tipoContenedor').val('');
		$('#caracteristicaContenedor').val('');
	}
});
