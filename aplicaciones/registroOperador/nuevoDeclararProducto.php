...
?>
					...
					<table id="operaciones" style="width:100%">
						<thead>
							<tr>
								...
								<th>Producto</th>
								<th>Actualizar organismo de certificación</th>
							</tr>
						</thead>
						<tbody>
						<?php 
							...
							while ($fila = pg_fetch_assoc($qProductos)){
								...
								echo $cr->imprimirLineaProductoOperacion($fila['id_operacion'], $fila['nombre_tipo_producto'], $fila['nombre_subtipo_producto'], $fila['nombre_producto'], $fila['id_producto'], 'NO', $eliminarProducto, $fila['id_operacion']);
							}
						?>
						</tbody>
					</table>
		...
		<form id="enviarProductosOperador" data-rutaAplicacion="registroOperador" data-destino="detalleItem" data-accionEnExito="ACTUALIZAR">
			...
			<div id="numeroOperacionProducto"></div>
...
<script type="text/javascript">
	...
	$("#enviarProductosOperador").submit(function(event){
		...
		if($("#operaciones >tbody >tr").find(".ingresoProducto").length !=0){
				...
				$("#operaciones > tbody > tr").find('input[name="agencia[]"]').each(function (i) {
				    if ($(this).prop('checked')) {
				        $("#numeroOperacionProducto").append('<input type="hidden" name="agenciaProducto[]" value ="' + $(this).val() + '"/>');
				    }
				});
