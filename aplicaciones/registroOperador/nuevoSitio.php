	...
	<fieldset>
		<legend>Información del Sitio</legend>
			...
			<label>Provincia</label>
				<select id="provinciaSitio" name="provincia">
			...
			<div data-linea="2">
			<label>Cantón</label>
				<select id="cantonSitio" name="canton" disabled="disabled">
				</select>
			</div>
			...
			<label>Código catastral</label> 
				<input type="text" id="codigo_catastral" name="codigo_catastral" placeholder="Ej: 5687845" maxlength="32" onblur="validarCodigoCatastral(this)"/>
			</div>
...
var longitud = -78.99169921875;
    ...
    $("#provinciaSitio").change(function(event){
    	scanton ='0';
		scanton = '<option value="">Cantón...</option>';
	    for(var i=0;i<array_canton.length;i++){
		    if ($("#provinciaSitio").val()==array_canton[i]['padre']){
	    ...
	    $('#cantonSitio').html(scanton);
	    $("#cantonSitio").removeAttr("disabled");
	    $("#codigoProvincia").val($("#provinciaSitio").val());
	    cargarValorDefecto("tipoArea","");
    ...
    $("#cantonSitio").change(function(){
		sparroquia ='0';
		sparroquia = '<option value="">Parroquia...</option>';
	    for(var i=0;i<array_parroquia.length;i++){
		    if ($("#cantonSitio").val()==array_parroquia[i]['padre']){
		    	sparroquia += '<option value="'+array_parroquia[i]['codigo']+'">'+array_parroquia[i]['nombre']+'</option>';
			    } 
	    	}

	    var valLat = $('#cantonSitio option:selected').attr('data-latitud');
	    var valLon = $('#cantonSitio option:selected').attr('data-longitud');
	    var zona = $('#cantonSitio option:selected').attr('data-zona');
	...
	function chequearCamposSitio(form){
		...
		if(!$.trim($("#provinciaSitio").val()) || !esCampoValido("#provinciaSitio")){
			error = true;
			$("#provinciaSitio").addClass("alertaCombo");
		}

		if(!$.trim($("#cantonSitio").val()) || !esCampoValido("#cantonSitio")){
			error = true;
			$("#cantonSitio").addClass("alertaCombo");
		}
		...
		if(!$.trim($("#codigo_catastral").val()) || !esCampoValido("#codigo_catastral")){
			error = true;
			$("#codigo_catastral").addClass("alertaCombo");
		}
...
/***
 * validar un codigo catastral ya no se encuentre en otro sitio
 */
 function validarCodigoCatastral(elemento){
 	if($(elemento).val() != ''){
 		$("#cargarMensajeTemporal").html("<div id='cargando'>Cargando...</div>");
 		$.post("aplicaciones/registroOperador/validarCodigoCatastral.php", 
             {
 		        codigo_catastral: $(elemento).val(),
         		identificador_operador: usuario
             }, function (data) {
             	$("#cargarMensajeTemporal").html("");
             	    if (data.estado === 'EXITO') {
             	    	$("#codigo_catastral").removeClass("alertaCombo");
 	                    mostrarMensaje(data.mensaje, data.estado);
 	                    distribuirLineas();
                     } else {
                     	mostrarMensaje(data.mensaje, "FALLO");
                     	$("#codigo_catastral").focus();
                     	$("#codigo_catastral").addClass("alertaCombo");
                     }
         }, 'json');
 	}
 	else{
 		mostrarMensaje("Código catastral no puede ser vacio!!", "FALLO");
     	$("#codigo_catastral").focus();
     	$("#codigo_catastral").addClass("alertaCombo");
 	}
}
