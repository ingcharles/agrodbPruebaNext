...
try{
	...
	$datosProductosActualizacion = $_POST['agenciaProducto'];
	...
	try{
		if ($operacion['id_producto'] != null){
			...
			//eliminar de la tabla operaciones_organico los productos ya asignados 
			if (is_array($datosProductosActualizacion) && count($datosProductosActualizacion) != 0){
				foreach ($datosProductosActualizacion as $datoOperacion){
						$cr->quitarOperacionesOrganico($conexion, $datoOperacion);
				}
			}
		...
		if ($tipoProceso){
			...
			switch ($idArea) {
				...
				case 'SA':
					switch ($opcionArea) {
						...
						case 'OPT':
                                ...
                                if(pg_num_rows($qOperaciones)>0){
                                    $modulosAgregados.="('PRG_CATAS_PRODU', 'PRG_ASIGNA_CUV'),";
                                    $perfilesAgregados.="('PFL_ADM_ACT_PORC', 'PFL_SOLICI_CUV'),";
                                }
                        break;
						case 'OPI':
							$qOperaciones=$cr->buscarOperacionesPorCodigoyAreaOperacion($conexion, $identificadorOperador,"('$opcionArea')","('$idArea')");
							...
							if(pg_num_rows($qOperaciones)>0){
								$modulosAgregados.="('PRG_ASIGNA_CUV')";
								$perfilesAgregados.="('PFL_SOLICI_CUV')";
							}
					break;
						case 'PRO':
				...
				case 'IAF':
					switch ($opcionArea) {
						...
						case 'IDE':
							...
							if($opcionArea == 'DIS'){
								$modulosAgregados.="('PRG_REG_PRODUCTO'),";
								$perfilesAgregados.="('PFL_OPE_REG_PRO'),";
							}
						break;
						case 'FIE':
						case 'IEX':
				...
				case 'CGRIA':
					switch ($opcionArea) {
						case 'ODI':
							$modulosAgregados.="('PRG_DOSSIER_PEC'),('PRG_ENSA_EFI_MVC'),";
							$perfilesAgregados.="('PFL_EE_OI'),('PFL_ORG_ENSA_EFI'),";
						break;
					}
				break;
				case 'IAP' :
				...
				case 'IAV':
				    switch ($opcionArea) {
				        ...
				        case 'FOR':
				        case 'FRA':
				           $modulosAgregados.="('PRG_PROV_EXTE'),('PRG_DOS_PEC_MVC'),";
				           $perfilesAgregados.="('PFL_USR_PROV_EXT'),('PFL_USR_DOS_PEC'),";
				        break;
				    }
				break;
			}
