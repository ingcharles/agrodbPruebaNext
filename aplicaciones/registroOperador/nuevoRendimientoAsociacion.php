...
<header>
				...
				if ($tipoActividad == "grupal" || $tipoActividad == "grupal-SIC"){					
					...
					if(pg_num_rows($qDatosMiembroAsociacion)>0){?>
						...
						<div data-linea="1">
							...
							<label style="font-weight: normal; color: darkred"> El código MAG es exclusivo para musáceas</label>
						</div>
	...
	echo '<table id="tRendimiento" width="100%">
				<tr><th width="25%">Producto</th><th width="25%">Superficie (ha)</th><th width="25%">Rendimiento</th><th width="25%">Actualizar organismo de certificación</th></tr>
				<input type="hidden" id="superficieInput" name="superficieInput[]" value="">
				<input type="hidden" id="rendimientoInput" name="rendimientoInput[]" value="">';
	...
	<label style="font-weight: normal; color: darkred"> Ingrese el área reportada en la lista integrada del certificado orgánico entregado bajo normativa nacional, en unidad de hectáreas. La suma de la superficie de cada producto debe ser menor o igual a la superficie del área total declarada. </label>
	';

	
	?>
	</fieldset>
	<p class="nota">Por favor, para registrar rendimientos, se debe realizar la conversión a “Toneladas/año” para cultivos, huevos y carne. Usar “kilogramos/año” para miel, miel cremosa, jalea real, propóleo, cera y polen. Utilizar “Litros/año” para bebidas; y “Número de unidades/año” para animales, colmenas y pilones. </br></br> Por ejemplo, si el certificado de banano se expide en cajas/año: </br> 1270 cajas época alta x 18,14 kg/caja= 23037,8 kg x 26 semanas= 598982,8 kg/1000 kg = 598,98 toneladas/año/época alta.  </br> 1005 cajas época baja x 18,14 kg/caja= 18230,7 kg x 26 semanas= 473998,2 kg/1000 kg = 473,99 toneladas/año/época baja. </br> Total: 598,98 + 473,99=1072,97 toneladas/año
	<p>
...
var tipo_actividad = <?php echo json_encode($tipoActividad); ?>;
	...
	function superficieRendimiento(){
		var arraySuperficie = {};
		$('input[name^="superficie["]').each(function() {
		  	var nombre = $(this).attr('name').replace(/\D/g, '');
		  	var valor = $(this).val();
		  	arraySuperficie[nombre] = valor;
		});
		$('#superficieInput').val(JSON.stringify(arraySuperficie));

		var arrayRendimiento = {};
		$('input[name^="rendimiento["]').each(function() {
			var nombre = $(this).attr('name').replace(/\D/g, '');
			var valor = $(this).val();
			arrayRendimiento[nombre] = valor;
		});
		$('#rendimientoInput').val(JSON.stringify(arrayRendimiento));
	}
