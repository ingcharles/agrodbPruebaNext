...
<div id="estado"></div>
	...
	<fieldset>
		...
		<div data-linea="6">
			<label>Código catastral: </label><?php echo $operadorSitio['codigo_catastral']; ?> 
		</div>
	...
	foreach ($idGrupoOperaciones as $solicitud){
		...
		if($idArea=='AI'){
		    ...
		    $seccionProductosAgenciaCertificadora .= "";
		    ...
		    switch ($opcionArea){
				...
				case 'REC':
		            ...
		            while ($fila = pg_fetch_assoc($productos)) {
		            	if($fila['nombre_subtipo'] == 'Musáceas'){
		            		$banderaMagap = true;
		            	}
		            }

		            if($banderaMagap){
		            	$cabeceraTablaProductos = '<th>Superficie (ha)</th> <th>MAG</th> <th>Rendimiento (t/año)</th>';
		            }else{
		    ...
		    if($opcionArea == "PRO" || $opcionArea == "REC" || $opcionArea == "PRC" || $opcionArea == "COM"){
				...
				if($opcionArea == "PRO" || $opcionArea == "REC"){
					...
					if(pg_num_rows($productos) > 0){
						...
						while ($fila = pg_fetch_assoc($productos)){
							if($banderaMagap){
								$seccionProductos .= '<tr><td>'.++$contadorProductos.'</td><td>'.$fila['nombre_tipo'].'</td><td>'.$fila['nombre_subtipo'].'</td><td>'.$fila['nombre_comun'].'</td><td>'.$fila['superficie_miembro'].'</td><td>'.$fila['codigo_magap'].'</td><td>'.$fila['rendimiento'].'</td></tr>';
							}else{
				...
				//productos ya asignados a un organismo certificador 
				if($opcionArea == "PRO")
				{
					$productosAsignados = $cr->obtenerProductosOrganicosPorIdOperadorTipoOperacionHistoricoAsignados($conexion, $idOperadorTipoOperacion, $historialOperacion['id_historial_operacion'], $operacion['estado']);
					if(pg_num_rows($productosAsignados) > 0 )
					{

						$seccionProductosAgenciaCertificadora .= '<fieldset>
							<legend>Datos de productos con organismo certificador</legend>';
						$banderaMagap = false;
				            while ($fila = pg_fetch_assoc($productosAsignados)) {
				            	if($fila['nombre_subtipo'] == 'Musáceas'){
				            		$banderaMagap = true;
				            	}
				            }
			            pg_result_seek($productosAsignados, 0);
						while ($fila = pg_fetch_assoc($productosAsignados))
						{
								if($banderaMagap){
									$cabeceraTablaProductosAsignados = '<th>Superficie (ha)</th> <th>MAG</th> <th>Rendimiento (t/año)</th> <th>Organimos certificador</th>';
									$seccionProductos .= '<tr><td>'.++$contadorProductos.'</td><td>'.$fila['nombre_tipo'].'</td><td>'.$fila['nombre_subtipo'].'</td><td>'.$fila['nombre_comun'].'</td><td>'.$fila['superficie_miembro'].'</td><td>'.$fila['codigo_magap'].'</td><td>'.$fila['rendimiento'].'</td><td>'.$fila['nombre_agencia_certificadora'].'</td></tr>';
								}else{
									$cabeceraTablaProductosAsignados = '<th>Superficie (ha)</th> <th>Rendimiento (t/año)</th> <th>Organimos certificador</th>';
									$seccionProductos .= '<tr><td>'.++$contadorProductos.'</td><td>'.$fila['nombre_tipo'].'</td><td>'.$fila['nombre_subtipo'].'</td><td>'.$fila['nombre_comun'].'</td><td>'.$fila['superficie_miembro'].'</td><td>'.$fila['rendimiento'].'</td><td>'.$fila['nombre_agencia_certificadora'].'</td></tr>';
								}	
								$productoCertificado[] = array('nombre'=>$operadorSitio['nombre_lugar'].'-'.$fila['nombre_comun'], 'idProducto'=>$fila['id_producto'], 'codigoSitio'=>$operadorSitio['id_sitio'], 'idOperacion'=>$fila['id_operacion'] );
						}

						$seccionProductosAgenciaCertificadora .=  "<table style='width: 100%''>
															<thead>
																<tr>
																	<th>#</th>
																	<th>Tipo producto</th>
																	<th>Subtipo producto</th>
																	<th>Producto</th>
																	" .$cabeceraTablaProductosAsignados ."
																</tr>
															</thead>
															<tbody>";
						...
						if(!$banderaProductoCertificado)
						{
						    while ($fila = pg_fetch_assoc($productosAsignados))
						    {
						        $seccionProductosAgenciaCertificadora .= '<tr><td>'.++$contadorProductos.'</td><td>'.$fila['nombre_tipo'].'</td><td>'.$fila['nombre_subtipo'].'</td><td>'.$fila['nombre_comun'].'</td></tr>';			      
						    }
						}else{
						    $seccionProductosAgenciaCertificadora .=  $seccionProductos;
						}
						$seccionProductosAgenciaCertificadora .=  "</tbody></table></fieldset>";
					}
				}

				if($opcionArea == "PRC")
				{
					$contadorProductosProcesador = 0;
					$productosProcesadorAsignados = $cr->obtenerProductosAreaSitioPorIdOperadorTipoOperacionHistoricoAsignados($conexion, $idOperadorTipoOperacion, $historialOperacion['id_historial_operacion'], $operacion['estado']);

					if(pg_num_rows($productosProcesadorAsignados) > 0 )
					{
						$seccionProductosAgenciaCertificadora .= '<fieldset>
							<legend>Datos de productos con organismo certificador</legend>';
						...
						$seccionProductosAgenciaCertificadora .=  "<table style='width: 100%''>
															...
															<tbody>";
						while ($fila = pg_fetch_assoc($productosProcesadorAsignados))
					    {
					        $seccionProductosAgenciaCertificadora .= '<tr><td>'.++$contadorProductosProcesador.'</td><td>'.$fila['nombre_tipo'].'</td><td>'.$fila['nombre_subtipo'].'</td><td>'.$fila['nombre_comun'].'</td><td>'.$fila['nombre_agencia_certificadora'].'</td></tr>';			      
					    }
				...
				if($banderaMostrarAsignacion){
					...
					$seccionAgenciaCertificadora .= '<fieldset>
								<legend>Datos de organismo certificador</legend>
								<div data-linea="7">
					...
					if($banderaValidacionAI == 'COMERCIALIZADOR'){
						...
						$qPaises = $cc->listarPaisesMasUnionEuropea($conexion);
	...
	echo $seccionProductosAgenciaCertificadora;
	...
	<?php
		$qRutaCertificado = $cr->obtenerRutaCertificado($conexion, $operacion['identificador_operador'], $operacion['id_tipo_operacion']);
		$rutaCertificado = pg_fetch_assoc($qRutaCertificado);
		if(isset($rutaCertificado['ruta_poa']) && $rutaCertificado['ruta_poa'] != ''){
			echo '<fieldset>
					<legend>Último certificado POA</legend>
					<div data-linea="1"><label>Descargar: </label><a href="'.$rutaCertificado['ruta_poa'].'" target="_blank">Link para descargar certificado</a></div>
				</fieldset>';
		}
	?>
...
// << Fin material reproductivo
		...
		if (error){
			...
			if($("#mercadoDestino").val()!=""){
				...
				if($("#detallesMercado #r_"+$("#mercadoDestino").val()).length==0){
					...
					if($("#mercadoDestino").val() == '2062'){
						if(agregoPaisUE() == false){
							desabilitarPaises();
							...
							mostrarMensaje("Quitar los paises pertecientes a la Unión Europea previamente agregados","FALLO");
						}
	...
	var paisesUnionEuropea = ['Alemania', 'Bélgica', 'Croacia', 'Dinamarca', 'España', 'Francia', 'Irlanda', 'Letonia', 'Luxembourg', 'Paises Bajos', 'Suecia', 'Bulgaria', 'Eslovaquia', 'Estonia', 'Grecia', 'Malta', 'Polonia', 'República Checa', 'Austria', 'Chipre', 'Eslovenia', 'Finlandia', 'Hungría', 'Italia', 'Lituania', 'Portugal','Rumanía'];
	function desabilitarPaises(){
		$("#mercadoDestino > option").each(function() {
		    if(jQuery.inArray($(this).text(), paisesUnionEuropea) != -1){
		    	$(this).prop('disabled', true);
		    }
		});
	}

	function habilitarPaises(){
		$("#mercadoDestino > option").each(function() {
		    	$(this).prop('disabled', false);
		});
	}
	...
	function agregoPaisUE(){
		var existe = false;
		$('#detallesMercado > tr > td:first-child').each(function(){ 
			if(jQuery.inArray($(this).text(), paisesUnionEuropea) != -1){
		    	existe = true;
		    }
		});
