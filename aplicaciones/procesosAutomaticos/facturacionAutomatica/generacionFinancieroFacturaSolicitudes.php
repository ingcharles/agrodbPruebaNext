...
//if(1){
	...
	$resultadoMonitoreo = $cm->obtenerCronPorCodigoEstado($conexion, 'CRON_FACT_PG_ELEC_SOL');
	...
	//if(1){
		...
		$datosFactura = $cfa->obtenerDatosGenerarFacturaAutomaticoSolicitudes($conexion, " in ('Por atender', 'Por atender saldo')");				   
		...
		while ($listaFactura = pg_fetch_assoc($datosFactura)){
			...
			switch ($cabeceraOrdenPago['tipo_solicitud']){
			    ...
			    case 'certificadoFito':			        
					...
					$codigoCertificadoFitosanitario = $certificadoFitosanitario['codigo_certificado_fitosanitario'];																					   
	...
	$arch = fopen("../../../aplicaciones/logs/cron/automatico_financiero_factura_solicitudes".date("d-m-Y").".txt", "a+");
