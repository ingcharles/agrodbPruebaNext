		...
		while ($fila = pg_fetch_assoc($datosPorGenerar)){
		    ...
		    switch ($fila['tipo_solicitud']){      
				...
				case 'certificadoFito':
					...
					$qDatosProvincia = $ccf->obtenerDatosProvinciaPorIdCertificadoFitosanitario($conexion, $idCertificadoFitosanitario);
		            $datosProvincia = pg_fetch_assoc($qDatosProvincia);
		            $provinciaSolicitud = $datosProvincia['nombre_provincia'];
				...
				case 'modificacionProductoRia':
					...
					$qDatosProvincia = $cmp->obtenerDatosSolicitudProvincia($conexion, $idSolicitudProducto);
				    $datosProvincia = pg_fetch_assoc($qDatosProvincia);
				    $provinciaSolicitud = $datosProvincia['provincia'];
				...
				case 'registroProductoRia':
				    ...
				    $solicitudProducto = pg_fetch_assoc($crpr->abrirSolicitudPorNumeroSolicitud($conexion, $fila['id_vue']));
				    $identificadorCliente = $solicitudProducto['identificador_operador'];
				    $idSolicitudProducto = $solicitudProducto['id_solicitud_registro_producto'];
				    
					$qDatosProvincia = $crpr->abrirSolicitud($conexion, $idSolicitudProducto);
				    $datosProvincia = pg_fetch_assoc($qDatosProvincia);
				    $provinciaSolicitud = $datosProvincia['provincia_operador'];
				    
				    $tipoFormulario	= 'registroProductoRia';
				    $tipoInspector = 'Financiero';
				    $identificadorTabla = $idSolicitudProducto;
		    ...
		    $parameters['parametrosReporte'] = array(
				...
				'provinciaSolicitud' => $provinciaSolicitud
		    );
		    ...
		    switch ($fila['tipo_solicitud']){
				...
				case 'certificadoFito';
					...
					$cMail = new ControladorMail();
						...
						break;
		        case 'modificacionProductoRia':															
					$cmp->actualizarEstadoSolicitudPorIdSolicitudProducto($conexion, $identificadorTabla, 'verificacion');					
					break;
				case 'registroProductoRia':
					$crpr->actualizarEstadoSolicitudPorIdSolicitudRegistroProducto($conexion, $idSolicitudProducto, 'verificacion');
					break;
				default:
		            echo 'Tipo de solicitud no definida.';
		    }   
