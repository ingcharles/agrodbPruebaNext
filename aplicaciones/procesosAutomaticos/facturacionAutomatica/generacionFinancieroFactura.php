...
if($_SERVER['REMOTE_ADDR'] == ''){
	...
	if($resultadoMonitoreo){
		...
		$datosFactura = $cfa->obtenerDatosGenerarFacturaAutomatico($conexion, 'Por atender');

		while ($listaFactura = pg_fetch_assoc($datosFactura)){

			echo IN_MSG . $listaFactura['id_vue'];
			...
			if($listaFactura['tipo_proceso'] == 'factura'){
				...
				$cfa->actualizarEstadoFacturaFinancieroAutomaticoCabecera($conexion, $listaFactura['id_vue'], 'W');
					...
					if($datosInstitucion['fecha_caducidad_pfx'] >= $fechaActualSistema){
						...
						if($resultadoFirma == 'Firmado'){
						   ...
						   if (!file_exists('../../financiero/archivoXml/firmados/'.$rutaFecha.'/')){
							...
							$cfa->actualizarEstadoFacturaFinancieroAutomaticoCabecera($conexion, $listaFactura['id_vue'], 'Atendida');
