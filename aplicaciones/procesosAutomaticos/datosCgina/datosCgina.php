...
if ($_SERVER['REMOTE_ADDR'] == ''){
 ...
 $datosFruticula = $cCgina->obtenerIngredientesActivos($conexion, $fecha);
    ...
    /////// CULTIVOS ///////
		...
		echo '<br/> >>>>Inicio Obtencion de Centros de Acopio de Leche <br/>';
		$datosAcopioLeche = $cCgina->obtenerCentrosAcopioLeche($conexion, $fecha);

		$valoresNuevos = '';
		$contador=0;

		while ($filas = pg_fetch_row($datosAcopioLeche)){
			if($filas[20]==''){
				$filas[20]=0;
			}
			$valoresNuevos .= "('$filas[0]','$filas[1]','$filas[3]','$filas[4]',
								'$filas[5]','$filas[6]','$filas[7]','$filas[8]','$filas[9]',
								'$filas[10]','$filas[11]','$filas[12]','$filas[14]',
								'$filas[15]','$filas[20]','$filas[23]','$filas[24]','$filas[2]'
		),";

					
					$contador+=1;
		}
		echo'<br/> >>>>se Obtuvo '.$contador.' Centros de Acopio de Leche <br/>';

		$valoresNuevos = trim($valoresNuevos, ',');
		echo '<br/> >>>>Inicio Proceso de Inserción de Centros de Acopio de Leches en MAG <br/>';
		 if ($valoresNuevos != ''){
			echo '<br/> >>>>Inicio Inserción de Centros de Acopio de Leches  <br/>';
		 	$cCgina->insertarCentrosAcopioLeche($conexionCgina,$valoresNuevos,'centro_acopio_leche');
			//$cCgina->insertarCentrosAcopioLeche($conexion,$valoresNuevos,'centro_acopio_leche');
		 	
			echo '<br/> <<< Fin de ingreso de Centros de Acopio de Leche <br/>';
		 }
