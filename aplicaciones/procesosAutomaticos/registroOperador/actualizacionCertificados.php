...
//if(1){
	...
	//if(1){
    	...
    	while($operaciones = pg_fetch_assoc($qOperaciones)){ 	    
            ...
            $qInspectorProvincia = $cc->obtenerProvinciaInspector($conexion, $inspector);
            $inspectorProvincia = pg_fetch_result($qInspectorProvincia, 0 , 'provincia');
    	    ...
    	    switch ($idArea) {
    	        case 'AI':
    	            ...
    	            $fechaActualizacion = pg_fetch_result($cr->obtenerFechaActualizacionRegistroOrganicos($conexion, $idOperador, $idTipoOperacion), 0, 'fecha_actualizacion');
    	                ...
    	                case 'REC':
    	                    ...
    	                    if (pg_num_rows($qOperacionesXIdOperadorXIdTipoOperacion) > 0) {
    	                    	...
    	                    	if ($opcionArea == "PRO") {
    	                    	    ...
    	                    	    $reporteDos = "PRO2";
    	                    	    $reporteTres = "PRO3";
    	                    	    $codigoQr = 'http://181.112.155.163/' . $constg::RUTA_APLICACION . '/aplicaciones/registroOperador/certificados/certificadosPOA/PRO-' . $idOperador . '.pdf';
    	                    	    $salidaArchivoPoa = 'aplicaciones/registroOperador/certificados/certificadosPOA/PRO-'.$idOperador.'.pdf';
    	                    	} else if ($opcionArea == "REC") {
    	                    	    $reporteUno = "REC1";
    	                    	    $reporteDos = "REC2";
    	                    	    $reporteTres = "REC3";
    	                    	    $codigoQr = 'http://181.112.155.163/' . $constg::RUTA_APLICACION . '/aplicaciones/registroOperador/certificados/certificadosPOA/REC-' . $idOperador . '.pdf';
    	                        ...
    	                        $firmaResponsable = pg_fetch_assoc($cc->obtenerFirmasResponsablePorProvincia($conexion, $inspectorProvincia, 'AI'));
    	                ...
    	                case 'COM':
    	                    ...
    	                    if (pg_num_rows($qOperacionesXIdOperadorXIdTipoOperacion) > 0) {
    	                       ...
    	                       if(pg_num_rows($qBuscarCodigoPOA)==0){
								...
								$mercados = $cr->obtenerMercadosDestino($conexion, $idTipoOperacion, $idOperador);
                                $mercadosDestino = '';
                                while($mercado = pg_fetch_assoc($mercados)){
                                    $mercadosDestino = $mercadosDestino . $mercado['mercado_destino'] . ', ';
                                }
								
								$mercadosDestino = substr($mercadoDestino, 0, -2);
    	                        $reporteUno = "";
    	                        $reporteDos = "";
								$reporteTres = "";
    	                        ...
    	                        if ($opcionArea == 'COM') {
    	                            $reporteUno = 'COM1';
    	                            $reporteDos = 'COM2';
    	                            $codigoQr = 'http://181.112.155.163/' . $constg::RUTA_APLICACION . '/aplicaciones/registroOperador/certificados/certificadosPOA/COM-' . $idOperador . '.pdf';
									$salidaArchivoPoa = 'aplicaciones/registroOperador/certificados/certificadosPOA/COM-'.$idOperador.'.pdf';
    	                        } else if ($opcionArea == 'PRC') {
    	                            $reporteUno = 'PRC1';
    	                            $reporteDos = 'PRC2';
									$reporteTres = "PRC3";
    	                            $codigoQr = 'http://181.112.155.163/' . $constg::RUTA_APLICACION . '/aplicaciones/registroOperador/certificados/certificadosPOA/PRC-' . $idOperador . '.pdf';
    	                         ...
    	                         $parameters['parametrosReporte'] = array(
    	                        	...
    	                        	'fechaRegistroActualizacion'=> $fechaActualizacion,
    	                        	'codigoQr'=> $codigoQr,
                                    'mercadosDestino'=>$mercadosDestino
    	                        );
