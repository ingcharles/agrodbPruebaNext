...
// ***************************************** evaluacion primaria*******************************************************
			...
			if (isset($datos['evaluacionPrimariaInput'])){
				...
				if (isset($datos['evaluacionPrimariaInput'])){
					foreach ($datos['evaluacionPrimariaInput'] as $value){
				...
				if (isset($datos['evaluacionPrimariaTxtInput'])){
					foreach ($datos['evaluacionPrimariaTxtInput'] as $value){
...
// *********************************************************************registrar cambios***************************************************
	...
	public function buscarInformacionPaciente($arrayParametros){
		$consulta = "SELECT 
						identificador,nombre ||' '||apellido as funcionario, nombre, apellido, genero, estado_civil, fecha_nacimiento, edad, 
       					...
       					tiene_enfermedad_catastrofica, nombre_enfermedad_catastrofica,carnet_conadis_familiar, mail_personal, tipo_discapacidad, porcentaje_discapacidad,
	...
	public function obtenerDatosContrato($arrayParametros){
		$consulta = "SELECT 
						id_datos_contrato, oficina, direccion, coordinacion,nombre_puesto,(SELECT fecha_inicial FROM g_certificados_uath.devolver_fecha_inicial(dc.identificador) order by 1 ASC limit 1) as fecha_inicial,
						(SELECT jornada_laboral FROM g_uath.ficha_empleado fe WHERE fe.identificador=dc.identificador and estado_empleado ='activo') as jornada_laboral, actividades_relevantes
					FROM
	...
	public function obtenerDatosFirma($arrayParametros){
		$consulta = "SELECT 
			 ...
			 (SELECT nombre_puesto FROM g_uath.datos_contrato dc WHERE identificador=fe.identificador and estado = 1) as cargo
  			FROM 
			g_uath.ficha_empleado fe 
			WHERE fe.identificador ='" . $arrayParametros['identificador'] . "'";
		return $this->modeloHistoriaClinica->ejecutarSqlNativo($consulta);
	}

	public function obtenerDatosProfesional($arrayParametros){
		$consulta = "SELECT 
						fe.identificador, fe.nombre ||' '||fe.apellido as funcionario,
						fi.ruta_archivo as firma_medico,
						fi.clave as clave
					FROM 
					g_uath.ficha_empleado fe
					LEFT JOIN g_firma_documentos.firmantes fi ON fe.identificador = fi.identificador
					WHERE fe.identificador ='" . $arrayParametros['identificador'] . "' and estado_empleado ='activo'";
		return $this->modeloHistoriaClinica->ejecutarSqlNativo($consulta);
	}
