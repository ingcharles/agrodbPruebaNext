...
class OrdenPagoLogicaNegocio implements IModelo
{
	...
	public function buscarFacturasUsuario($arrayParametros)
	{
		...
		switch ($arrayParametros['tipo_solicitud']) {
			...
			case "certificadoFito":
				$tabla = 'g_financiero.orden_pago op';
				$busqueda = 'and op.tipo_solicitud = ' . "'" . $arrayParametros['tipo_solicitud'] . "'" . '
							and (' . $arrayParametros['numero_factura'] . ' is NULL or op.numero_factura = ' . $arrayParametros['numero_factura'] . ')
	                         and (' . $arrayParametros['numero_solicitud'] . ' is NULL or op.numero_solicitud = ' . $arrayParametros['numero_solicitud'] . ')';
		...
		$consulta = "SELECT
                        ...
                        op.numero_factura, op.total_pagar, op.estado_sri, to_char(op.fecha_facturacion,'DD/MM/YYYY') fecha_facturacion, op.estado_sri
					FROM
						$tabla
					WHERE
						...
						$busqueda " . "ORDER BY fecha_facturacion desc";
		//echo $consulta;
		return $this->modeloOrdenPago->ejecutarSqlNativo($consulta);
	}
     ...
     */
	public function generarOrdenPago($idPago, $rutaFecha, $nombreArchivo,$provincia){
	...
	public function buscarOrdenPagoRecargaSaldo($orden){
		...
		$consulta = "SELECT 
					...
					op.total_pagar, op.observacion, op.estado, op.localizacion, op.orden_pago, cl.razon_social, cl.direccion, 
					CASE WHEN fc.metodo_pago IS NULL THEN 'Depósito' ELSE fc.metodo_pago END as metodo_pago, op.factura
					FROM 
					g_financiero.orden_pago op
					INNER JOIN g_financiero.clientes cl ON op.identificador_operador = cl.identificador
					LEFT JOIN g_financiero_automatico.financiero_cabecera fc ON op.id_pago = fc.id_orden_pago
					WHERE
						id_pago='".$orden['orden']."'";
