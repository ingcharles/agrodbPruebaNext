...
use Agrodb\EmisionCertificacionOrigen\Controladores\RegistroProduccionControlador;
...
class BaseControlador extends Comun
{
	...
	*/
	...
	public function filtroEmision(){
	    ...
	    $this->panelBusqueda = '<table class="filtro" style="width: 100%;">
                                            	<tbody>
                            						...
                            						<tr  style="width: 100%;">
														<td><div style="color: red" id="alerta">Escoger por lo menos un campo marcado con (*)</div></td>
														<td colspan="4" style="text-align: end;">
															<button id="btnFiltrar">Filtrar lista</button>
														</td>
													</tr>
	...
	public function comboEspecie($especie=null){
		...
		$datosUsuario = new RegistroProduccionControlador();
		$cedulaUsuario = $datosUsuario->buscarEmpresaEmpleadoPorIdentificadorEmpleado($_SESSION['usuario']);
	...
	public function comboSitio($sitio=null){
		...
		$datosUsuario = new RegistroProduccionControlador();
		$cedulaUsuario = $datosUsuario->buscarEmpresaEmpleadoPorIdentificadorEmpleado($_SESSION['usuario']);
	    $arrayParametros = array('identificador_operador' => $cedulaUsuario, 'id_area_tipo_operacion' =>'AI', 'codigo' => 'FAE');
	...
	public function comboProduccionSubproducto($produccionSubproducto=null){
	...
	public function comboDestino1($opcion = null)
	{
		...
		if($_POST['opcion'] == 'null'){
			...
			$combo .= '<option value="' . $_POST['opcion'] . '">' . $_POST['opcion'] . '</option>';
		}
	...
	public function SitioCf($arrayParametros){
	    $lNegocioEmisionCertificado = new EmisionCertificadoLogicaNegocio();
	    $verificar = $lNegocioEmisionCertificado->buscarCentroFaenamientoSitioEmergente($arrayParametros);
	 ...
	 * Combo
	 */
	public function comboProductosOtenidos($opcion = null)
	{
	    $array = array("Canales sin restricción de uso","Canales para uso industrial");
	    $combo = '<option value="">Seleccionar....</option>';
	    foreach ($array as $item)
