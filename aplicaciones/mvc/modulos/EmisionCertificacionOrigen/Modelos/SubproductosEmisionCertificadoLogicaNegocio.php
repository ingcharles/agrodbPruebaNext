...
class SubproductosEmisionCertificadoLogicaNegocio implements IModelo 
{
	...
	*/
	...
	public function buscarSubproductosEmision($arrayParametros)
	{
		...
		if(array_key_exists('estado', $arrayParametros) && $arrayParametros['estado'] == 'creado'){
			if(array_key_exists('producto_movilizar', $arrayParametros) && (($arrayParametros['producto_movilizar'] == 'Canal') || ($arrayParametros['producto_movilizar'] == 'Canal con subproductos') )){
				$busqueda .= " and dec.fecha_creacion_produccion::text ilike ('%".$arrayParametros['fecha_creacion_produccion']."%')";
			}
		...
		if(array_key_exists('producto_movilizar', $arrayParametros) && $arrayParametros['producto_movilizar'] == 'Subproductos'){
			$busqueda .= " and dec.fecha_creacion_produccion::text ilike ('%".$arrayParametros['fecha_creacion_produccion']."%')";
		}
 	        ...
 	        $consulta = "SELECT
                     ...
                     where
                            ...
                            " . $busqueda . " and ec.estado in ('Vigente','Notificar','temporal') and dec.estado_detalle = 'activo'
	...
	public function obtenerIdEmisionCertificado($id_subproductos_emision_certificado)
	{
	    
	    $consulta = "SELECT det.id_emision_certificado FROM g_emision_certificacion_origen.subproductos_emision_certificado sub
					INNER JOIN g_emision_certificacion_origen.detalle_emision_certificado det
					ON sub.id_detalle_emision_certificado = det.id_detalle_emision_certificado
					WHERE sub.id_subproductos_emision_certificado = ".$id_subproductos_emision_certificado."";
