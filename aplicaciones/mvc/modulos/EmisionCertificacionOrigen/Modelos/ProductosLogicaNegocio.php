...
class ProductosLogicaNegocio implements IModelo 
{
	...
	*/
	...
	public function buscarCantidadProductos($arrayParametros){
		...
		if (isset($arrayParametros['fecha_creacion_produccion']) && $arrayParametros['fecha_creacion_produccion'] != ""){
			$fechaFormateada = date("Y-m-d", strtotime($arrayParametros['fecha_creacion_produccion']));
		}else{
			$fechaFormateada =$arrayParametros['fecha_faenamiento'];
		}
	        ...
	        $consulta = "
            	    ...
            	    WHERE
                        ...
                        and rp.identificador_operador ='" . $arrayParametros['cedulaUsuario'] . "'
                        ".$busqueda." and to_char( sub.fecha_creacion, 'YYYY-MM-DD') ilike ('%".$fechaFormateada."%');";
