...
<form id='formulario' data-rutaAplicacion='<?php echo URL_MVC_FOLDER; ?>EmisionCertificacionOrigen' 
	 ...
	 <input type="hidden" id="emergente" name="emergente" value="noEmergenteProduccion"/>
...
var tipo = <?php echo json_encode($this->tipo);?>; 
	...
	$("#fecha_faenamiento2").datepicker({
        ...
        onSelect: function(dateText, inst) {
        	...
        	var fecha=new Date($('#fecha_faenamiento2').datepicker('getDate'));
        	
    		fecha.setDate(fecha.getDate()-1);
    	  	fecha.setMonth(fecha.getMonth());
    		fecha.setUTCFullYear(fecha.getUTCFullYear());  
    		
    		$('#fecha_recepcion2').datepicker('option', 'minDate', fecha);
    		$('#fecha_recepcion2').datepicker('option', 'maxDate', fecha);
    		$('#fecha_recepcion2').datepicker( 'setDate', fecha );
        }
...
//fechas para las especies de Avicola,cunicola
	  $("#fecha_recepcion1").datepicker({
        ...
        onSelect: function(dateText, inst) {
    		...
    		$('#fecha_faenamiento1').datepicker('option', 'minDate', fechaMinima);
    		$('#fecha_faenamiento1').datepicker( 'setDate', fechaMinima );
        }
        
      });
	$("#fecha_faenamiento1").datepicker({
    	yearRange: "c:c",
    	changeMonth: false,
        changeYear: false,
        dateFormat: 'yy-mm-dd'
    		  ...
    		  $.post("<?php echo URL ?>EmisionCertificacionOrigen/registroProduccion/listarCanalObtenido", 
    	              {
    			         numCanalesObtenidos:$("#num_animales_recibidos").val(),
    	              	...
    	              	if (data.estado === 'EXITO') {
    	              		    ...
    	              		    $("#num_canales_uso_industri").html('<option value="">Seleccionar....</option>');
      		  ...
      		  $.post("<?php echo URL ?>EmisionCertificacionOrigen/registroProduccion/comboAnimalesRecibidos", 
      	              {
      	            	...
      	            	$("#cargarMensajeTemporal").html("");
      	              	if (data.estado === 'EXITO') {
      	                	...
      	                	$("#num_animales_recibidos").html('');
							$("#num_canales_obtenidos").html('');
      	   					$("#num_canales_obtenidos_uso").html('');
      	                  	mostrarMensaje(data.mensaje, "FALLO");
      	                  }
      	      }, 'json');
				$("#num_animales_recibidos").html('');
		...
		$( "#tipo_especie" ).change(function() {
			...
			if(especie !=''){
				...
				if(especie == "Avícola" || especie == "Porcinos" || especie == "Cunícola" || especie == "Cavia"){
					...
					$("#fecha_recepcion2").attr('disabled', 'disabled');
				}
			}else{
				$("#faenamientoContenedor").hide();
				$("#recepcionContenedor").hide();
