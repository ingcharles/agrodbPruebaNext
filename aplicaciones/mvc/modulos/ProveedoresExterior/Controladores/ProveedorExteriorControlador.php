...
class ProveedorExteriorControlador extends BaseControlador{
	...
	public function paGenerarCertificadoProveedorExterior(){
	     
	    $query = " id_proveedor_exterior in (1, 3, 5, 6, 7, 8, 9, 12, 13, 14, 16, 17, 18, 19, 20, 21, 23, 24, 25, 
                        26, 31, 35, 36, 37, 39, 40, 41, 42, 43, 44, 46, 47, 49, 50, 51, 52, 
                        53, 54, 55, 56, 57, 59, 63, 64, 67, 68, 69, 70, 71, 72, 75, 77, 78, 
                        80, 81, 82, 85, 86, 88, 89, 90, 91, 92, 93, 94, 107, 108, 109, 111, 
                        115, 116, 117, 118, 119, 120, 122, 124, 125, 126, 127, 128, 129, 134, 
                        135, 137, 139, 140, 162, 163, 164, 166, 202, 203, 204, 206, 207, 208, 
                        209, 211, 212, 214, 216, 217, 220, 221, 222, 223, 225, 228, 233)";
	    //$query = " estado_proveedor_exterior = 'Aprobado'";
	    
	    $solicitudes = $this->lNegocioProveedorExterior->buscarLista($query);
	    
	    foreach ($solicitudes as $fila) {
	        
	        $idProveedorExterior = $fila['id_proveedor_exterior'];
	        $idCertificado = md5(rand());
	           
	        $this->lNegocioProveedorExterior->generarCertificadoProveedorExterior($idProveedorExterior, $idCertificado);
	        
	        echo 'Solicitud ' . $fila['id_proveedor_exterior']. ' generada \n';
	    }
