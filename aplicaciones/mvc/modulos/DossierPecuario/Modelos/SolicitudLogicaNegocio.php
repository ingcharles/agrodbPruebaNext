...
class SolicitudLogicaNegocio implements IModelo
{
    ...
    public function generarDocumentoCertificado($idSolicitud)
    {
        ...
        //Si el tipo es snack o cosmetico firma el Coordinador de Registros
        if($solicitud->getCodificacionSubtipoProducto() == 'SNK' || $solicitud->getCodificacionSubtipoProducto() == 'COS' || $solicitud->getCodificacionSubtipoProducto() == 'ADSP'){
    ...
    public function actualizarSolicitud($datos)
    {
        ...
        if (isset($datos['estado_solicitud'])) {
            ...
            switch ($datos['estado_solicitud']) {
                ...
                case 'Creado':
                    {
                        ...
                        if ($revision['error'] == true) { // Hay error, faltan campos/grids
                            ...
                            if ($datos['codificacion_subtipo_producto'] == 'SNK' || $datos['codificacion_subtipo_producto'] == 'ADSP' || $datos['codificacion_subtipo_producto'] == 'COS') {//|| $datos['tipo_solicitud'] == 'Modificacion'
                            ...
                            $ruta = $this->generarDocumentoExpediente($datos['id_solicitud']); //esto debe estar descomentado
                            $datos['ruta_expediente'] = $ruta;//esto debe estar descomentado
                ...
                case 'Aprobado':
                    {
                        $validacion = $this->verificarFirmaResponsable($datos['id_solicitud']);
                        //$datos['estado_solicitud'] = 'EnTramite';
        ...
        if ($validacion['bandera']) {
            ...
            //buscar el registro actualizado, verificar el numero de codigo final
            //buscar en ria productos con ese codigo
            //$validacion = $this->crearNuevoProductoRIA($datos['id_solicitud']);//<-----------------------------borrar!!!!!!!!!!!!!!!!!!!!!!!!!!!
            if (isset($datos['estado_solicitud']) && $datos['estado_solicitud'] == 'Aprobado') {
                ...
                if($validacion['bandera']){
                    ...
                    //Actualizar número de producto en dossier
                    $arrayCodigo = array(
                        'id_solicitud' => $datos['id_solicitud'],
                        'codigo_producto_final' => $validacion['codigo_producto'],
                        'ruta_certificado' => DOSS_PEC_URL . $solicitud->getIdExpediente() . "/" . "Certificado_" . $solicitud->getIdExpediente() . ".pdf",
                        'ruta_puntos_minimos' => DOSS_PEC_URL . $solicitud->getIdExpediente() . "/" . "PuntosMinimos_" . $solicitud->getIdExpediente() . ".pdf"
                    );
                    ...
                    if ($datos['tipo_solicitud'] != 'Registro') {
                        ...
                        if ($solicitud->getCambioTitular() == 'Si') {
                            $arrayCodigo['identificador'] = $solicitud->getIdentificadorTitular();
                        }
                    }
                    
                    $this->guardar($arrayCodigo);
                    /////////////////////////////
                    
                    // Crear Certificado
                    $ruta = $this->generarDocumentoCertificado($datos['id_solicitud']); //esto debe estar descomentado
                    // Crear Puntos Minimos
                    $ruta = $this->generarDocumentoPuntosMinimos($datos['id_solicitud']); //esto debe estar descomentado
                    // Crear Expediente
                    //$ruta = $this->generarDocumentoExpediente($datos['id_solicitud']);
                }else{
                    //rollback en estado de solicitud a EnTramite
                    
                    $arrayCodigo = array(
                        'id_solicitud' => $datos['id_solicitud'],
                        'estado_solicitud' => 'EnTramite'
                    );
    ...
    public function crearNuevoProductoRIA($idSolicitud)
    {
        ...
        //bloque de cambio en ingreso ria
        
        // Tabla productos
        $arrayProducto = array(
            'id_subtipo_producto' => $solicitud->id_subtipo_producto,
            'nombre_comun' => $solicitud->nombre_producto,
        ...
        if ($idProducto != null) {
            ...
            $idProductoInocuidad = $this->lNegocioProductosInocuidad->guardarProductoRIA($arrayProductoInocuidad, $solicitud->tipo_solicitud, $solicitud->codificacion_subtipo_producto);
            ...
            if ($idProductoInocuidad > 0) {
                ...
                //rollback
                //borrar la cabecera del producto!!!!!
                $this->lNegocioProductos->borrar($idProducto);
            }
            ...
            if ($idProductoInocuidad >0) {
                ...
                if ($solicitud->tipo_solicitud == 'Registro') {
                    ...
                    if (isset($productoInocuidad->current()->numero_registro) && $productoInocuidad->current()->numero_registro != '') {
                        
                        $validacion['codigo_producto'] = $productoInocuidad->current()->numero_registro;
                        $arrayProducto['ruta'] = DOSS_PEC_URL . $solicitud->id_expediente . "/" . "PuntosMinimos_" . $solicitud->id_expediente . ".pdf";
                        
                        if($solicitud->codificacion_subtipo_producto == 'FM'){
                            $numeroRegistroProducto = $productoInocuidad->current()->numero_registro;
                            $arrayProducto['nombre_comun'] = $numeroRegistroProducto;                            
                        }                            
                    }
                }else{
                    $arrayProducto['ruta'] = DOSS_PEC_URL . $solicitud->id_expediente . "/" . "PuntosMinimos_" . $solicitud->id_expediente . ".pdf";
                    
                    if (isset($solicitud->codigo_producto_final) && $solicitud->codigo_producto_final != '') {                        
                        $validacion['codigo_producto'] = $productoInocuidad->current()->numero_registro;
                    }
                }
                ...
                if ($validacion['estado'] == "exito") {
                    ...
                    $validacion['estado'] = "Fallo";
                    $validacion['mensaje'] .= " No se pudo guardar la información dedel Producto en RIA - rollback. ";
                    $validacion['bandera'] = false;
                    
                    //Se hace un rollback del producto
                    //borrar la cabecera del producto!!!!!
                    $this->lNegocioProductos->borrar($idProducto);
                    // Eliminar información de tablas anexas del producto RIA
                    $this->lNegocioComposicionInocuidad->borrarTodo($idProducto);
     ...
     * Función para crear el PDF del certificado
     */
    public function verificarFirmaResponsable($idSolicitud)
    {
        $validacion = array(
            'bandera' => false,
            'estado' => "Fallo",
            'mensaje' => "El firmante de la provincia no se encuentra registrado, gestionar el registro de firma electrónica en GUIA",
            'contenido' => null
        );
        ...
        //Verificar si el usuario se encuentra dentro de esquema firmantes
        $sentencia = "identificador='".$nombreCoordinador->current()->identificador."' and estado='Activo' LIMIT 1;";
        
        $usuario = $this->lNegocioFirmantes->buscarLista($sentencia);
        
        if (! empty($usuario->current())) {
            $validacion['estado'] = "exito";
            $validacion['mensaje'] = "Se ha encontrado el firmante.";
            $validacion['bandera'] = TRUE;
        } else {
            $validacion['estado'] = "Fallo";
            $validacion['mensaje'] = "El firmante de la provincia no se encuentra registrado, gestionar el registro de firma electrónica en GUIA";
            $validacion['bandera'] = FALSE;
        }
    ...
    public function verificarEliminarSolicitud($idSolicitud)
    {
        $validacion = array(
            'bandera' => false,
            'estado' => "Fallo",
            'mensaje' => "Ocurrió un error al eliminar la solicitud de dossier",
            'contenido' => null
        );        
        
        $resultado = $this->eliminarSolicitudes($idSolicitud);
            
        if ($resultado) {
            $validacion['estado'] = "exito";
            $validacion['mensaje'] = "Se ha eliminado el registro.";
            $validacion['bandera'] = true;
        } else {
            $validacion['estado'] = "Fallo";
            $validacion['mensaje'] = "Ocurrió un error al eliminar la solicitud de dossier.";
            $validacion['bandera'] = false;
        }
     ...
     * @return array|ResultSet
     */
    public function eliminarSolicitudes($idSolicitud)
    {
        $consulta = "UPDATE 
                        g_dossier_pecuario_mvc.solicitud
                	SET 
                        estado_solicitud='Eliminado', 
                        identificador_revisor='".$_SESSION['usuario']."', 
                        fase_revision='Administrador', 
                        fecha_revision=now(), 
                        observacion_revision='Eliminado por administrador'
                	WHERE 
    ...
    public function actualizarSolicitudCron($datos)
    {
        $validacion = array(
            'bandera' => true,
            'estado' => "exito",
        ...
        if($validacion['bandera']){
            $datos['identificador_revisor'] = 'GUIA';
            $datos['fase_revision'] = 'Aprobacion CRON';
            $datos['fecha_revision'] = 'now()';
            $datos['estado_solicitud'] = 'Aprobado';
            $datos['observacion_revision'] = 'Aprobado por proceso automatico GUIA';
            
            // Función de registro de histórico
            $this->lNegocioSecuenciaRevision->guardarHistoricoAprobacionAutomatica($datos['id_solicitud'], $datos['estado_solicitud'], 'Aprobado por proceso automatico GUIA');
            
            $validacion['bandera'] = true;
        }else{
            echo 'Solicitud '. $datos['id_solicitud'] . ': '. $validacion['estado'] . ' - ' . $validacion['mensaje'];
            $validacion['bandera'] = false;
        }
        //}
        ...
        if ($validacion['bandera']) {
            ...
            //if (isset($datos['estado_solicitud']) && $datos['estado_solicitud'] == 'Aprobado') {
                ...
                if($validacion['bandera']){
                    ...
                    if ($solicitud->getTipoSolicitud() != 'Registro') {
                        // Cambia de estado a la solicitud de origen
                        $idSolicitudOriginal = $solicitud->getIdSolicitudOriginal();
                    ...
                    //rollback en estado de solicitud a Pagado
                    ...
                    echo 'Solicitud '. $datos['id_solicitud'] . ': '. 'Error de guardado de producto RIA. '. $validacion['estado'] . ' - ' . $validacion['mensaje'];
                }
            ...
            $validacion = array(
                'bandera' => true,
                'estado' => "Fallo",
                'mensaje' => "Ocurrió un error en procesar el producto en los catálogos RIA y los certificados, revisar datos creados y hacer reverso.",
                'contenido' => null
            );
        }
