...
class SolicitudControlador extends BaseControlador
{
    ...
    public function abrir()    
    {
        ...
        if($this->modeloSolicitud->getEstadoSolicitud() == "verificacion"){
            $query = "id_solicitud = '".$this->modeloSolicitud->getIdSolicitud()."' and tipo_solicitud='dossierPecuario' and estado=3";
    ...
    /**
     * Construye el listado de solicitudes a eliminar
     */
    public function listaSolicitudesEliminar($idSolicitud)
    {
        $sentencia = "id_solicitud in (" . $idSolicitud . ") and estado_solicitud not in ('Eliminado', 'Aprobado', 'Rechazado', 'Pagado') order by id_expediente ASC;";
        $solicitudes = $this->lNegocioSolicitud->buscarLista($sentencia);
        
        $listaSolicitud = '';
        $i=1;
        
        foreach ($solicitudes as $item){
            $listaSolicitud .=' <div data-linea="'.$i++.'">
                        			<label>Id expediente: </label>'.$item->id_expediente.'                                    
                        		</div>
                                <div data-linea="'.$i++.'">
                        			<label>Nombre producto: </label>'.$item->nombre_producto.'                                   
                        		</div>
                                <hr />
                                ';
            
            $this->solicitudes .= $item->id_solicitud .',';
        }
        
        $this->solicitudes = substr($this->solicitudes, 0, -1);
    ...
    public function confirmarEliminacion()
    {
        ...
        if($_POST['elementos'] != null){
            $this->formulario = "Eliminar";
            $this->eliminar = $this->listaSolicitudesEliminar($_POST['elementos']);
        }else{
    ...
    /**
     * Método para borrar un registro en la base de datos de manera lógica- Solicitud
     */
    public function eliminarSolicitud()
    {
        $resultado = $this->lNegocioSolicitud->verificarEliminarSolicitud($_POST['id']);
    ...
    public function articleHtmlSolicitudes()
    {
        ...
        foreach ($consultaCabecera as $fila1) {
            ...
            switch ($fila1['estado_solicitud']) {
				...
				case 'Pagado':
                    $this->article .= "<h2> Solicitudes pagadas pendientes de aprobación </h2>";
                    $opcion = 'abrir';
                    break;
    ...
    public function comboEstadosRevisionDossierPecuarioTecnico($opcion=null)
    {
        ...
        if ($opcion == "pago") {
            $combo .= '<option value="pago" selected="selected">Aprobado</option>';
            $combo .= '<option value="Rechazado">Rechazado</option>';
            $combo .= '<option value="Subsanacion">Subsanación</option>';
        }else if ($opcion == "Rechazado") {
            $combo .= '<option value="pago">Aprobado</option>';
     ...
     */
    public function paAprobacionSolicitudes(){
        
        echo "\n".'Proceso Automático de cambio de estado de solicitudes Dossier Pecuario'."\n"."\n";
        
        $consulta = "estado_solicitud = 'Pagado'";
        ...
        foreach ($solicitudes as $fila) {
            ...
            $validacion = $this->lNegocioSolicitud->actualizarSolicitudCron($arrayParametros);
            ...
            if($validacion['estado'] == 'exito'){
                ...
                $this->lNegocioSolicitud->guardar($arrayParametros);
                echo 'El dossier pecuario ' . $fila['id_solicitud']. ' - ' . $fila['nombre_producto']. ' cambia de estado a Aprobado'."\n";
            }else{
                $arrayParametros = array(
                    'id_solicitud' => $fila['id_solicitud'],
                    'estado_solicitud' => 'ErrorCron'
                );
                $this->lNegocioSolicitud->guardar($arrayParametros);
                
                echo 'El dossier pecuario ' . $fila['id_solicitud']. ' - ' . $fila['nombre_producto']. ' ' . $validacion['estado'] . ' - ' . $validacion['mensaje'];
            }
        }
        echo "\n";
    }
