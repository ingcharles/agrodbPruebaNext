		...
		if ($arrayToken['estado'] == 'exito') {
			...
			$consulta = "SELECT row_to_json (res) as res FROM ( SELECT array_to_json(array_agg(row_to_json(listado))) as cuerpo FROM (
				...
				FROM
					...
					LEFT JOIN (SELECT substring(MAX (codigo_muestra) FROM '([1-9]\d*)')::bigint AS secuencialorden, codigo_trampa_padre FROM f_inspeccion.moscaf01_detalle_ordenes GROUP BY codigo_trampa_padre) tso ON tso.codigo_trampa_padre = adt.codigo_trampa
