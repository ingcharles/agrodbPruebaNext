...
class Controlf02LogicaNegocio implements IModelo 
{
	...
	*/
	 ...
	 * @return array|ResultSet
	 */
	public function obtenerTransitoIngreso($provincia) {
		...
		if($arrayToken['estado'] == 'exito'){
			...
			if (trim($provincia) != '' && $provincia != null ) {
				$inner_provincia = "INNER JOIN g_catalogos.puertos pu ON cf02.id_punto_salida::Integer = pu.id_puerto";
				$busqueda_provincia = "and upper(pu.nombre_provincia) = upper('$provincia')";
			}
	...
	public function guardarIngresos(Array $arrayIngresos, Array $arrayProductos) {
		...
		if($arrayToken['estado'] == 'exito'){
			...
			try{
				...
				echo json_encode(array('estado' => 'error', 'mensaje' => $ex->getMessage()));
				$procesoIngreso->rollback();
				throw new GuardarExcepcion($ex, array('origen' => 'Agro servicios', 'ws'=>'RestWsTransitoIngresoControlador', 'archivo' => 'Controlf02LogicaNegocio', 'metodo' => 'guardarIngresos', 'datos' => $arrayIngresos));
			}
