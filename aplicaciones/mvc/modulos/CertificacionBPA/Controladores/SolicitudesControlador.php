...
class SolicitudesControlador extends BaseControlador
{
    ...
    {        
        ...
        foreach ($consultaCabecera as $fila1) {
            ...
            switch ($fila1['estado'])
            {
				...
				case 'Anulado':
                    $this->article .="<h2> Solicitudes Anuladas </h2>";
                    break;
    ...
    public function comboTipoCertificadoEquivalente()
    {
        ...
        if ($opcion == "Global Gap") {
            ...
            $combo .= '<option value="Flor Ecuador">Flor Ecuador</option>';
            $combo .= '<option value="Certificación Orgánica">Certificación Orgánica</option>';
            $combo .= '<option value="NFCRS-Cocoa">NFCRS-Cocoa</option>';
        } else if ($opcion == "Flor Ecuador") {
            $combo .= '<option value="Global Gap">Global Gap</option>';
            $combo .= '<option value="Flor Ecuador" selected="selected">Flor Ecuador</option>';
            $combo .= '<option value="Certificación Orgánica">Certificación Orgánica</option>';
            ...
            $combo .= '<option value="Certificación Orgánica" selected="selected">Certificación Orgánica</option>';
            $combo .= '<option value="NFCRS-Cocoa">NFCRS-Cocoa</option>';
        } else if ($opcion == "NFCRS-Cocoa") {
            $combo .= '<option value="Global Gap">Global Gap</option>';
            $combo .= '<option value="Flor Ecuador">Flor Ecuador</option>';
            $combo .= '<option value="Certificación Orgánica">Certificación Orgánica</option>';
            $combo .= '<option value="NFCRS-Cocoa" selected="selected">NFCRS-Cocoa</option>';
        } else {
            $combo .= '<option value="Global Gap">Global Gap</option>';
            $combo .= '<option value="Flor Ecuador">Flor Ecuador</option>';
            $combo .= '<option value="Certificación Orgánica">Certificación Orgánica</option>';
    ...
    */
   public function listaSolicitudesAnular($idSolicitud)
   {
       $sentencia = "id_solicitud in (" . $idSolicitud . ") and estado in ('Aprobado') order by id_solicitud ASC;";
       $solicitudes = $this->lNegocioSolicitudes->buscarLista($sentencia);
       ...
       foreach ($solicitudes as $item){
           $listaSolicitud .=' <div data-linea="'.$i++.'">
                        			<label>Id solicitud: </label>'.$item->id_solicitud.'
                        		</div>
                                <div data-linea="'.$i++.'">
                        			<label>Nombre operador: </label>'.$item->razon_social.'
                        		</div>
                                <hr />
                                ';
       }
   ...
   {
       $this->accion = "Anular Solicitud de Certificación BPA";
       
       if($_POST['elementos'] != null){
           $this->formulario = "Anular";
           $this->anular = $this->listaSolicitudesAnular($_POST['elementos']);
       }else{
           $this->formulario = "Error";
       }
       
       require APP . 'CertificacionBPA/vistas/formularioConfirmacionAnulacionSolicitudVista.php';
   }
   ...
   {
       $resultado = $this->lNegocioSolicitudes->verificarAnularSolicitud($_POST['id']);
