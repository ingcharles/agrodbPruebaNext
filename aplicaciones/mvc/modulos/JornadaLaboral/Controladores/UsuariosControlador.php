...
class UsuariosControlador extends BaseControlador{
	...
	public function editar(){
		...
		}else if ($perfilAdministrador == "PFL_ADMIN_USU_GU" && ($tipoUsuario->current()->codificacion_perfil == 'PFL_REGIST_OPERA' || $tipoUsuario->current()->codificacion_perfil == 'PFL_USUAR_EXT')){
			$this->banderaUsuario = 'SI';
		}else{
			if ($tipoUsuario->current()->codificacion_perfil == 'PFL_REGIST_OPERA' || $tipoUsuario->current()->codificacion_perfil == 'PFL_USUAR_EXT'){
				$this->mensajeBanderaUsuario = 'Operadores o Usuarios externos';
