			...
			if($procesoValidacion[0]){
				...
				for ($i = 0; $i < count($datos['id_importacion_producto']); $i ++){
					...
					if(isset($datos['codigo_presentacion'][$i])){
					    ...
					    } else if ($datos['id_area'] == 'IAP'){
					        $lNegocioPresentacionesPlaguicidas = new PresentacionesPlaguicidasLogicaNegocio();
					        $nombrePresentacion = $lNegocioPresentacionesPlaguicidas->buscarPresentacionProductoImportacion($datos['id_producto'][$i], $datos['codigo_presentacion'][$i], $datos['partida_arancelaria'][$i], $datos['codigo_complementario_suplementario'][$i]);
					    }
