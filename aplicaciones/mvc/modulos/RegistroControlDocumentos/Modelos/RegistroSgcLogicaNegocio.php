...
class RegistroSgcLogicaNegocio implements IModelo{
	...
	public function filtrarReporteRegistrosSocializacion(Array $arrayParemetros = array()){
	    $busqueda = '';
	    ...
	    $consulta = "SELECT 
						rs.id_registro_sgc, numero_memorando, numero_glpi, 
						rs.coordinacion, 
						(select
										nombre
								from
										g_estructura.area
								where
										id_area = rs.coordinacion
								limit 1) as nombre_coordinacion,
						subproceso, 
						(select
										nombre
								from
								...
								where
										id_area = subproceso
								limit 1) as nombre_subproceso,
						nombre_area, formato, nombre_documento, fecha_aprobacion, 
						fecha_notificacion, dd.identificador,
						
						 
						fecha_vigencia, dd.estado, edicion, resolucion, observacion , rs.estado_registro, dd.id_detalle_destinatario ,
						
						ds.id_detalle_socializacion, 
						numero_memorando, t.nombre, fecha_socializacion, nombre_socializar, documento_socializar
					FROM 
						g_registro_control_documentos.detalle_destinatario dd 
						inner join g_registro_control_documentos.registro_sgc rs on dd.id_registro_sgc = rs.id_registro_sgc						 
						full outer join g_registro_control_documentos.detalle_socializacion ds on ds.id_detalle_destinatario=dd.id_detalle_destinatario 
						full outer join g_registro_control_documentos.tecnico t on t.id_tecnico = ds.id_tecnico
					where 
						rs.socializar = 'Si' and rs.estado not in ('temporal') 
						" . $busqueda . "
					ORDER BY id_detalle_destinatario ASC";
	...
	public function crearExcelSocializacion($arrayDatos, $arrayParametros){
	    $documento = new ReportesExcelModelo();
	    $documento->getProperties()
	    ->setCreator("GUIA")
	    ->setLastModifiedBy('GUIA')
	    ...
	    foreach ($arrayParametros as $items){
	        ...
	        if ($items['documento_socializar'] != null){
	            $url = Constantes::RUTA_DOMINIO . '/' . Constantes::RUTA_APLICACION . '/' . $items['documento_socializar'];
	        }else{
	            $url="";
	        }
	        
	        $documento->cuerpoDinamicoHorizontal($contador, $items['id_registro_sgc'], 'ffffff', 1, 10, 0, 1, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['numero_memorando'], 'ffffff', 1, 10, 0, 2, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['numero_glpi'], 'ffffff', 1, 10, 0, 3, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['nombre_coordinacion'], 'ffffff', 1, 10, 0, 4, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['nombre_subproceso'], 'ffffff', 1, 10, 0, 5, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['formato'], 'ffffff', 1, 10, 0, 6, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['nombre_documento'], 'ffffff', 1, 10, 0, 7, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['fecha_aprobacion'], 'ffffff', 1, 10, 0, 8, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['fecha_notificacion'], 'ffffff', 1, 10, 0, 9, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['nombre_area'], 'ffffff', 1, 10, 0, 10, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['fecha_socializacion'], 'ffffff', 1, 10, 0, 11, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $items['estado'], 'ffffff', 1, 10, 0, 12, 0);
	        $documento->cuerpoDinamicoHorizontal($contador, $url, 'ffffff', 1, 10, 0, 13, 0);
	        $contador ++;
	        $item ++;
	    }
