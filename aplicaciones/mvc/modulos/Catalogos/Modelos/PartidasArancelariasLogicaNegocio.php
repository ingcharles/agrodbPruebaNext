...
class PartidasArancelariasLogicaNegocio implements IModelo 
{
	...
	*/
	 ...
	 * @return ResultSet
	 */
	public function comboPartidaProductoPlaguicidaImportacion($idProducto){
	    
	    $selectProducto = '';
	    
	    $query = " id_producto = $idProducto and estado='activo'";
	    
	    $partidas = $this->buscarLista($query);

	    foreach($partidas as $fila){
	        $selectProducto .= '<option value="' . $fila['partida_arancelaria'] . '" >' .  $fila['partida_arancelaria'] . '</option>';	        
	    }
	 ...
	 * @return ResultSet
	 */
	public function comboCodigoPartidaProductoPlaguicidaImportacion($idProducto){
	    ...
	    foreach($partidas as $fila){
	        $selectCodigoProducto .= '<option value="A' . $fila['codigo_producto'] . '" >' .  $fila['partida_arancelaria'] . ' - A' .$fila['codigo_producto'] . '</option>';
	    }
