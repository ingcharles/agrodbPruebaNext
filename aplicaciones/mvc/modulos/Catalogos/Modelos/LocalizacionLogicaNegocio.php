...
class LocalizacionLogicaNegocio implements IModelo{
	 ...
	 * @return ResultSet Registro categoria=0
	 */
	public function buscarPaisesPorIdPorIdioma($idPais, $idioma){
		if ($idioma == 'SPA'){
			$clave = 'nombre';
			$where = "id_localizacion = '$idPais' and categoria = 0";
		}else{
			$clave = 'nombre_ingles';
			$where = "id_localizacion = '$idPais' and categoria = 0";
		}
	...
	public function obtenerOficinasXCanton($idCanton){
		$consulta = "SELECT * from g_catalogos.localizacion 
		where categoria = 3 and id_localizacion_padre = $idCanton order by 3;";
		 return $this->modeloLocalizacion->ejecutarSqlNativo($consulta);
	}
}
