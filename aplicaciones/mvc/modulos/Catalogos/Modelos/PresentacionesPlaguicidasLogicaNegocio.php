...
class PresentacionesPlaguicidasLogicaNegocio implements IModelo
{
     ...
     * @return ResultSet
     */
    public function comboPresentacionesCodigoCompSuplPartidaProductoPlaguicidaImportacion($idProducto){
        ...
        $consulta = "SELECT
                    ...
                    FROM
                        ...
                        INNER JOIN g_catalogos.presentaciones_plaguicidas pp ON pp.id_codigo_comp_supl = ccs.id_codigo_comp_supl
                    WHERE
        ...
        $partidas = $this->modeloPresentacionesPlaguicidas->ejecutarSqlNativo($consulta);
        
        foreach($partidas as $fila){
            $selectCodigoProducto .= '<option value="' . $fila->codigo_presentacion . '" >' .  $fila->partida_arancelaria . ' - ' .$fila->codigo_complementario.$fila->codigo_suplementario . ' - ' .$fila->presentacion.' '.$fila->unidad . '</option>';
        }
     ...
     * @return ResultSet
     */
    public function buscarPresentacionProductoImportacion($idProducto, $codigoPresentacion, $partidaArancelaria, $codigoComplSupl){
        
        $codigoComplementario = substr($codigoComplSupl, 0, 4);
        $codigoSuplementario = substr($codigoComplSupl, 4, 4);
        ...
        $consulta = "SELECT
                    ...
                    WHERE
						...
						ccs.codigo_complementario = '$codigoComplementario' and
						ccs.codigo_suplementario = '$codigoSuplementario' and
                        pp.codigo_presentacion = '$codigoPresentacion' and
                    	pa.estado='activo' and
                    	ccs.estado='activo' and
	                    pp.estado = 'activo';";
