...
class OperadoresLogicaNegocio implements IModelo{
	...
	public function obtenerDatosOperadores($arrayParametros) {
		...
		$consulta ="SELECT
						...
						, CASE WHEN o.nombre_tecnico = '' THEN 'S/R' ELSE o.nombre_tecnico || ' ' || o.apellido_tecnico END as representante_tecnico
                    FROM
						g_operadores.operadores o
	...
	/**
	 * Ejecuta una consulta(SQL) personalizada, para obtener la información de un operador
	 * que posea operaciones de productos RIA en un área determinada.
	 *
	 * @return array|ResultSet
	 */
	public function obtenerDatosOperadorPorIdentificadorOperadorPorTipoSolicitudRia($arrayParametros) {
	    
	    $identificadorOperador = $arrayParametros['identificador_operador'];
	    $idArea = $arrayParametros['id_area'];
	    
	    $consulta = "SELECT 
                        	distinct (ttop.nombre) as operaciones_registradas
                        	, op.identificador_operador
                        	, COALESCE(o.razon_social, o.nombre_representante ||' '|| o.apellido_representante) as nombre_razon_social
                        	, ttop.codigo
                        FROM
                        	...
                        	INNER JOIN (SELECT 
                        				...
                        				WHERE
                            				top.id_area = '" . $idArea . "'
                            				and top.estado = 1) ttop ON op.id_tipo_operacion = ttop.id_tipo_operacion
                        WHERE
