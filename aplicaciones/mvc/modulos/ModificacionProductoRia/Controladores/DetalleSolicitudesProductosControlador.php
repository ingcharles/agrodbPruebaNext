        ...
        if ($proceso) {
            ...
            switch ($tipoModificacion) {
                    ...
                    switch ($idArea) {
                        ...
                        case 'IAV':
                            ...
                            $arrayParametros = array(
                                ...
                                'nombre_especie' => trim($nombreEspecie)
                            );
				...
				case "modificarClasificacionProducto":
                    ...
                    $subtipoProducto = $_POST['subtipo_producto'];
                    
                    $arrayParametros = array(
                        'id_detalle_solicitud_producto' => $idDetalleSolicitudProducto
                    );
                    
                    $verificarClasificacionProducto = $this->lNegocioClasificacionProducto->buscarLista($arrayParametros);
                    
                    if (count($verificarClasificacionProducto) == 0) {
                        
                        $datosClasificacionProducto = array(
                            'id_tabla_origen' => $idSubtipoProducto,
                            'clasificacion_producto' => $subtipoProducto,
                            'id_detalle_solicitud_producto' => $idDetalleSolicitudProducto
                        );
                        
                        $idClasificacionProducto = $this->lNegocioClasificacionProducto->guardar($datosClasificacionProducto);
                        
                        $clasificacionProductoControlador = new ClasificacionProductoControlador();
                        
                        $filaClasificacionProducto = $clasificacionProductoControlador->generarFilaClasificacionProducto($idClasificacionProducto, $datosClasificacionProducto, $tiempoAtencion);
                        
                        echo json_encode(array(
                            'validacion' => $validacion,
                            'resultado' => $resultado,
                            'filaClasificacionProducto' => $filaClasificacionProducto
                        ));
                    } else {
                        $validacion = "Fallo";
                        $resultado = "Solo puede agregar una clasificación de producto.";
                        echo json_encode(array(
                            'validacion' => $validacion,
                            'resultado' => $resultado
                        ));
