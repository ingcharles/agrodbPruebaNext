...
class SolicitudesProductosLogicaNegocio implements IModelo
{
    ...
    public function guardarFinalizarSolitud(array $datos)
    {
        try {
            ...
            if (isset($datos['estado_actual_solicitud_producto']) && $datos['estado_actual_solicitud_producto'] == "subsanacion") {

                if (!empty($datos['array_ruta_documento_respaldo'])) {

                    $arrayDocumentoRespaldo = $datos['array_ruta_documento_respaldo'];

                    $statement = $this->modeloSolicitudesProductos->getAdapter()
                        ->getDriver()
                        ->createStatement();

                    foreach ($arrayDocumentoRespaldo as $item) {

                        $datosDetalle = array('id_detalle_solicitud_producto' => (integer)$item['idDetalleSolicitudProducto'],
                            'ruta_documento_respaldo' => $item['rutaDocumentoRespaldo']);

                        $sqlActualizar = $this->modeloSolicitudesProductos->actualizarSql('detalle_solicitudes_productos', $this->modeloSolicitudesProductos->getEsquema());
                        $sqlActualizar->set($datosDetalle);
                        $sqlActualizar->where(array('id_detalle_solicitud_producto' => (integer)$item['idDetalleSolicitudProducto']));
                        $sqlActualizar->prepareStatement($this->modeloSolicitudesProductos->getAdapter(), $statement);
                        $statement->execute();
    ...
    public function buscarSolicitudesProductoXFiltro($arrayParametros)
    {
        ...
        if (isset($tipoSolicitud) && ($tipoSolicitud != '')) {
            $busqueda .= " and psp.id_area = '" . $tipoSolicitud . "'";
        }
     ...
     * @return array|ResultSet
     */
    public function obtenerSolicitudesProductos($arrayParametros)
    {

        $idArea = $arrayParametros['idArea'] != "" ? "'" . $arrayParametros['idArea'] . "'" : "NULL";
        ...
        $consulta = "SELECT
					 ...
					 WHERE 
                        ($identificador is NULL or sp.identificador_operador = $identificador)
                        and ($numeroSolicitud is NULL or sp.numero_solicitud = $numeroSolicitud)
                        and ($estadoSolicitud is NULL or sp.estado_solicitud_producto = $estadoSolicitud)
                        and ($idArea is NULL or sp.id_area = $idArea)
                        and ($fecha is NULL or to_char(sp.fecha_creacion,'YYYY-MM-DD') = $fecha)  
    ...
    public function actualizarDatosRevisionTecnica($arrayParametros)
    {
        ...
        $rutaVerificacionFirma = $arrayParametros['ruta_verificacion_firma'] === '' ? null : $arrayParametros['ruta_verificacion_firma'];
        ...
        $consulta = "UPDATE
                    ...
                    SET
                    	...
                    	ruta_verificacion_firma = '" . $rutaVerificacionFirma . "',
                    	" . $fechaActualizar . "
                    WHERE
                    	id_solicitud_producto = '" . $idSolicitudProducto . "';";
                ...
                case 'IAV':
                    switch ($modificacion) {
                        ...
                        case 'modificarVidaUtil':
                            ...
                            $datosActualizacion = [
                                'id_producto' => $idProducto,
                                'periodo_reingreso' => $datosVidaUtil->current()->estabilidad,
                            ];
                        ...
                        case 'modificarClasificacionProducto':
                            $lNegocioProductos = new ProductosLogicaNegocio();
                            $lNegocioClasificacionProducto = new ClasificacionProductoLogicaNegocio();

                            $datosClasificacionProducto = $lNegocioClasificacionProducto->buscarLista($datosModificacion);

                            $datosActualizacion = [
                                'id_producto' => $idProducto,
                                'id_subtipo_producto' => $datosClasificacionProducto->current()->id_tabla_origen,
                            ];
        ...
        switch ($idArea) {
            case 'IAP':
                ...
                $parametros = [
                    ...
                    'rutaFondo' => Constantes::RUTA_DOMINIO . '/' . Constantes::RUTA_APLICACION . '/aplicaciones/general/img/fondoCertificado.png'
                ];
