...
use Agrodb\Catalogos\Modelos\AditivoToxicologicoLogicaNegocio;
use Agrodb\Catalogos\Modelos\TiposProduccionFitosanitariasLogicaNegocio;
use ErrorException;
use Agrodb\Financiero\Modelos\DistritosLogicaNegocio;
...
class Comun{
	...
	public function comboTipoSolicitudFinanciero(){
		...
		$combo .= '<option value="certificadoFito">Certificado Fitosanitario</option>';																		 
	...
	public function comboUnidadesMedida($codigoUnidadMedida = null){
		...
		$query = "estado='Activo' 
                    and clasificacion  != 'sercop'
                    and tipo_unidad not in ('tiempo') order by nombre ASC";				   
	...
	public function comboPuertosPorIdentificador($idPais, $idPuerto = null, $codigoPais = null){
		...
		if (isset($idPais)){
			$query = " id_pais = '" . $idPais . "' ORDER BY nombre_puerto ASC";
		}

		if (isset($codigoPais)){
			$query = " codigo_pais = '" . $codigoPais . "' ORDER BY nombre_puerto ASC";
		}
		
		$combo = $lNegocioPuerto->buscarLista($query);

		foreach ($combo as $item){
			if ($idPuerto == $item->id_puerto){
				$comboPuertos .= '<option value="' . $item->id_puerto . '" data-nombre="' . $item->nombre_puerto . '" selected>' . $item->nombre_puerto . '-[' . $item->codigo_puerto . ']</option>';
			}else{
				$comboPuertos .= '<option value="' . $item->id_puerto . '" data-nombre="' . $item->nombre_puerto . '">' . $item->nombre_puerto . '-[' . $item->codigo_puerto . ']</option>';
			}
		}
		return $comboPuertos;
	}
	 ...
	 * @return string
	 */
	public function comboPaisesPorIdioma($idioma, $idLocalizacion = null, $banderaVisualizar = true){
		$localizacion = new LocalizacionLogicaNegocio();
	...
	public function comboTiposTratamientoPorIdioma($idioma, $idTipoTratamiento = null){
		...
		$query = "estado_tipo_tratamiento = 'Activo' ORDER BY nombre_tipo_tratamiento ASC";
	...
	public function comboTratamientosPorIdioma($idioma, $idTratamiento = null){
		...
		$query = "estado_tratamiento = 'Activo' ORDER BY nombre_tratamiento ASC";
	...
	*/
	...
	/**
	 * Consulta los subtipos de producto por id subtipo producto
	 *
	 * @param Integer $idTipoProducto
	 * @return string
	 */
	public function comboSubtipoProductoPorIdSubtipoProducto($idProducto, $idSubtipoProducto)
    {

        $subtipoProducto = new SubtipoProductosLogicaNegocio();
        $subtipos = "";

        $combo = $subtipoProducto->buscarSubtipoProductoPorIdSubtipoProducto($idProducto);

        foreach ($combo as $item) {
            if ($item->id_subtipo_producto == $idSubtipoProducto) {
                $subtipos .= '<option value="' . $item->id_subtipo_producto . '" data-nombre="' . $item->nombre . '" selected>' . $item->nombre . '</option>';
            } else {
                $subtipos .= '<option value="' . $item->id_subtipo_producto . '" data-nombre="' . $item->nombre . '">' . $item->nombre . '</option>';
            }
        }
	...
	public function provinciaXNombre($nombreProvincia){
		$localizacion = new LocalizacionLogicaNegocio();
		$provincia = $localizacion->buscarProvinciaXNombre($nombreProvincia);
		return $provincia;
	}
	
	/**
     * Select de tipo de solicitud de ensayo de eficacia
     *
     * @param Integer $idProducto
     * @return string
     */
    public function comboTipoSolicitudEnsayoEficacia($tipoSolicitud = null)
    {
        $selectTipoSolicitud = "";
        $arrayTipoSolicitud = array(
            'registro' => 'Registro',
            'ampliacionCondicionesUso' => 'Ampliación/Cambio en las condiciones de uso',
            'modificacionDosis' => 'Modificación de dosis',
            'reevaluacion' => 'Revaluación',
            'usoAdicionalregistroProceso' => 'Uso adicional para registro en proceso'
        );

        foreach ($arrayTipoSolicitud as $llaveTipoSolicitud => $valorTipoSolicitud) {

            if ($tipoSolicitud == $llaveTipoSolicitud) {
                $selectTipoSolicitud .= '<option value="' . $llaveTipoSolicitud . '" selected>' . $valorTipoSolicitud . '</option>';
            } else {
                $selectTipoSolicitud .= '<option value="' . $llaveTipoSolicitud . '" >' . $valorTipoSolicitud . '</option>';
            }

        }

        return $selectTipoSolicitud;
    }
     ...
     * @return string
     */
    public function comboTipoProductoEnsayoEficacia($tipoProducto = null)
    {
        $selectTipoProducto = "";
        ...
        foreach ($arrayTipoProducto as $llaveTipoProducto => $valorTipoProducto) {

            if ($tipoProducto == $llaveTipoProducto) {
                $selectTipoProducto .= '<option value="' . $llaveTipoProducto . '" selected>' . $valorTipoProducto . '</option>';
            } else {
                $selectTipoProducto .= '<option value="' . $llaveTipoProducto . '" >' . $valorTipoProducto . '</option>';
            }
	...
	/**
     * Consulta las unidades de medida utilizadas en procesos fitosanitarios 
     * y construye el combo
     * @param Integer $idUnidadMedida
     * @return string
     */
    public function comboUnidadesFitosanitaras($tipoUnidad = 'Medida', $codigoUnidad = null, $estatico = false){
        $unidadMedida = new UnidadesFitosanitariasLogicaNegocio();
        $unidades = "";
        
        $query = "estado_unidad_fitosanitaria = 'Activo'
                    and tipo_unidad_fitosanitaria = '". $tipoUnidad ."' 
                    ORDER BY nombre_unidad_fitosanitaria ASC";
        ...
        foreach ($combo as $item){
            ...
            if ($estatico){
                if ($codigoUnidad == $item['codigo_unidad_fitosanitaria']){
                    $unidades .= '<option value="' . $item->id_unidad_fitosanitaria . '" data-codigounidadmedida="' . $item->codigo_unidad_fitosanitaria . '" selected>' . $item->nombre_unidad_fitosanitaria . '</option>';
                }                
            }else{            
                if ($codigoUnidad == $item['codigo_unidad_fitosanitaria']){
                    $unidades .= '<option value="' . $item->id_unidad_fitosanitaria . '" data-codigounidadmedida="' . $item->codigo_unidad_fitosanitaria . '" selected>' . $item->nombre_unidad_fitosanitaria . '</option>';
                }else{
                    $unidades .= '<option value="' . $item->id_unidad_fitosanitaria . '" data-codigounidadmedida="' . $item->codigo_unidad_fitosanitaria . '" >' . $item->nombre_unidad_fitosanitaria . '</option>';
                }            
            }
    ...
    /**
     * Consulta los puertos relacionados con un pais por id de puerto por provincia
     */
    public function comboPuertosPorIdProvincia($idPais, $idProvincia = null){
        $lNegocioPuerto = new PuertosLogicaNegocio();
        ...
        if (isset($idProvincia)){
            $provincia = " and id_provincia = $idProvincia";
        }
        
        $query = "id_pais = '" .$idPais ."'" . $provincia . " and id_provincia is NOT NULL ORDER BY nombre_puerto ASC";
        
        $combo = $lNegocioPuerto->buscarLista($query);
        
        foreach ($combo as $item){
            if ($idProvincia == $item->id_provincia){
                $comboPuertos .= '<option value="' . $item->id_puerto . '" data-nombre="' . $item->nombre_puerto . '" selected>' . $item->nombre_puerto . ' - [' . $item->codigo_puerto . ']</option>';
            }else{
                $comboPuertos .= '<option value="' . $item->id_puerto . '" data-nombre="' . $item->nombre_puerto . '">' . $item->nombre_puerto . ' - [' . $item->codigo_puerto . ']</option>';
            }
	...
	/**
     * Consulta los aditivos por área y construye el combo
     */
    public function comboAditivoToxicologico($idArea, $idAditivo = null)
    {
        $aditivoToxicologico = new AditivoToxicologicoLogicaNegocio();
        
        $query = "area = '$idArea' ORDER BY area, nombre_comun ASC";
        $aditivo = "";
        
        $combo = $aditivoToxicologico->buscarLista($query);
        
        foreach ($combo as $item) {
            if ($idAditivo == $item->id_aditivo_toxicologico) {
                $aditivo .= '<option value="' . $item->id_aditivo_toxicologico . '" selected>' . $item->nombre_comun . '</option>';
            } else {
                $aditivo .= '<option value="' . $item->id_aditivo_toxicologico . '">' . $item->nombre_comun . '</option>';
            }
        }
        return $aditivo;
    }
	...
	/**
     * Consulta tipos de produccion para CFE y construye el combo
     */
    public function comboTipoProduccionFitosanitariasPorIdioma($idioma, $idtipoProduccion = null)
    {
    	
    	$tipoProduccionFitosanitaria = new TiposProduccionFitosanitariasLogicaNegocio();
    	
    	$nombreTipoProduccion = "";
    	$tipoProduccion = "";
    	
    	if($idioma == 'SPA'){
    		$nombreTipoProduccion = "nombre_tipo_produccion_fitosanitaria";
    	}
    	
    	if($idioma == 'ENG'){
    		$nombreTipoProduccion = "nombre_tipo_produccion_fitosanitaria_ingles";
    	}
    	
    	$query = "estado_tipo_produccion_fitosanitaria = 'Activo' order by " . $nombreTipoProduccion . " ASC";
    	
    	$combo = $tipoProduccionFitosanitaria->buscarLista($query);
    	
    	foreach ($combo as $item){
    		if ($idtipoProduccion == $item['id_tipo_produccion_fitosanitaria']){
    			if ($idioma == 'SPA'){
    				$tipoProduccion .= '<option value="' . $item->id_tipo_produccion_fitosanitaria . '" selected>' . $item->nombre_tipo_produccion_fitosanitaria . '</option>';
    			}
    			
    			if($idioma == 'ENG'){
    				$tipoProduccion .= '<option value="' . $item->id_tipo_produccion_fitosanitaria . '" selected>' . $item->nombre_tipo_produccion_fitosanitaria_ingles . '</option>';
    			}
    		}else{
    			if ($idioma == 'SPA'){
    				$tipoProduccion .= '<option value="' . $item->id_tipo_produccion_fitosanitaria . '" >' . $item->nombre_tipo_produccion_fitosanitaria . '</option>';
    			}
    			
    			if($idioma == 'ENG'){
    				$tipoProduccion .= '<option value="' . $item->id_tipo_produccion_fitosanitaria . '" >' . $item->nombre_tipo_produccion_fitosanitaria_ingles . '</option>';
    			}
    		}
    	}
	 ...
	 * @return string
	 */
	public function comboTipoProductoPorArea($idArea = null){
	...
	/**
	 * Consulta los rucs de Agrocalidad
	 *
	 * @param string $idArea
	 * @return string
	 */
	public function comboRucInstitucion(){
	    
	    $rucInstitucion = new DistritosLogicaNegocio();
	    $ruc = "";
	    
	    //$query = 'estado = "activo" oder by ruc ASC';
	    
	    $combo = $rucInstitucion->buscarLista();
	    
	    foreach ($combo as $item){
	        $ruc .= '<option value="' . $item->ruc . '">' . $item->ruc . '</option>';
	    }
	...
	/**
	 * Consulta los numeros de establecimiento por ruc
	 */
	public function comboNumeroEstablecimientoPorRuc($ruc = null)
	{

	    $numeroEstablecimientoInstitucion = new OficinaRecaudacionLogicaNegocio();
	    $numeroEstablecimiento = "";
	    	    
	    $combo = $numeroEstablecimientoInstitucion->buscarNumeroEstablecimientoPorRuc($ruc);
	    
	    foreach ($combo as $item){
	        $numeroEstablecimiento .= '<option value="' . $item->numero_establecimiento . '">' . $item->numero_establecimiento. '</option>';
	    }
