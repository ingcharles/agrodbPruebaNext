	...
	<?php 
		...
		while($fila = pg_fetch_assoc($res)){
			...
			$contenido = '<article 
					...
					<span>'.(strlen($asunto)>20?(mb_substr($asunto,0,20).'...'):(strlen($asunto)>0?$asunto:'Sin asunto')).'</span>
...
<script>
	$(document).ready(function(){
		...
		$("#detalleItem").html('<div class="mensajeInicial">Arrastre aquun elemento para visualizarlo.</div>');
	});
	</script>