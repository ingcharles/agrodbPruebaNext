	...
	//print_r($_SESSION);
		...
		while($vehiculo = pg_fetch_assoc($res)){
			...
			$contenido = '<tr 
					...
					<td><span class="n'.($vehiculo['estado']==1?'Vehiculo':($vehiculo['estado']==2?'Mantenimiento':($vehiculo['estado']==3?'Movilizacion':($vehiculo['estado']==4?'Siniestro':($vehiculo['estado']==5?'Matricula':'Rechazado'))))).'"></span></td>			
