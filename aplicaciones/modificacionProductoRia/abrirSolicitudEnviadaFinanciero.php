...
<fieldset>
    ...
    <div data-linea="5">
        <label>Representante
            Legal: </label> <?php echo $filaSolicitud['nombre_representante'] . ' ' . $filaSolicitud['apellido_representante']; ?>
    </div>
...
<fieldset>
    ...
    <div data-linea="9">
        <label>Motivo de
            Descuento: </label> <?php echo ($filaSolicitud['descuento'] == 'Si') ? 'Persona natural de la tercera edad o artesano' : 'N/A'; ?>
    </div>
</fieldset>
...
<fieldset>
    <legend>Modificaciones solicitadas</legend>
    <ul>
        <?php
        $qSolicitudDetalleProducto = $cmp->abrirDetalleSolicitudProducto($conexion, $idSolicitud);

        while ($fila = pg_fetch_assoc($qSolicitudDetalleProducto)) {
            echo '<li>' . $fila['tipo_modificacion'] . '</li>';
        }
        ?>
    </ul>
