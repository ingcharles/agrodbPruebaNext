...
try{
	...
	try {
		...
		if($datos['tipoProductoClv'] =='IAV'){
		    if(count($especie) != 0 || count($usoProducto) != 0){
				...
				if(pg_num_rows($composicionProducto) != 0){
				    ...
				    if(count($presentacion) != 0){
				        ...
				        $mensaje['estado'] = 'error';
				        $mensaje['mensaje'] = 'Debe seleccionar por lo menos una presentación para el producto.';
				    }
				...
				$mensaje['estado'] = 'error';
				$mensaje['mensaje'] = 'Debe seleccionar por lo menos una especie o uso.';
			}
		}															   
