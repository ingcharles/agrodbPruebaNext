	    ...
	    $res = $conexion->ejecutarConsulta("SELECT 
                                            ...
                                            FROM (
                                            	SELECT
													...
													(SELECT array_to_json(array_agg(row_to_json(r_p)))
                                                		FROM (
                                                			...
                                                			FROM
                                                				...
                                                				INNER JOIN g_catalogos.fabricante_formulador ff ON ff.id_fabricante_formulador = m.id_fabricante_formulador
                                                			WHERE
                                                    ...
                                                    to_char(pin.fecha_modificacion,'dd/mm/yyyy')::date as fecha_modificacion,
                                                    to_char(pin.fecha_revaluacion,'dd/mm/yyyy')::date as fecha_revaluacion,
												...
												GROUP BY pin.id_producto, pin.numero_registro, pin.id_operador, o.razon_social, epp.razon_social, p.nombre_comun, pin.fecha_registro, p.id_producto,
                                        		...
                                        		pin.observacion, tp.nombre, pin.fecha_modificacion, pin.fecha_revaluacion, pin.declaracion_venta
                                            ) as productosRPIP;");
		...
		$res = $conexion->ejecutarConsulta("SELECT row_to_json(productosRPIP)
			                                FROM (
												SELECT
													...
													FROM (
														SELECT
															nombre  || ' ' || pais_origen as formulador						  
														FROM
															g_catalogos.fabricante_formulador
														WHERE
															id_producto =  p.id_producto
...
public function reporteImprimirProductosInocuidadVeterinaria($conexion)
	...
	/************* REPORTE PRODUCTO PLAGUICIDA TOTAL **************/
	public function reporteProductoPlaguicidaVUECompleto($conexion)
	{
	    $res = $conexion->ejecutarConsulta("SELECT
                                            ...
                                            FROM
                                            	g_catalogos.productos p
												INNER JOIN g_catalogos.subtipo_productos st ON st.id_subtipo_producto = p.id_subtipo_producto
												INNER JOIN g_catalogos.tipo_productos t ON st.id_tipo_producto = t.id_tipo_producto
                                            	INNER JOIN g_catalogos.partidas_arancelarias pa ON p.id_producto = pa.id_producto
                                            	INNER JOIN g_catalogos.codigos_comp_supl ccs ON pa.id_partida_arancelaria = ccs.id_partida_arancelaria
                                            	INNER JOIN g_catalogos.presentaciones_plaguicidas pp ON ccs.id_codigo_comp_supl = pp.id_codigo_comp_supl
                                            WHERE
	...
	public function reporteProductoPresentacionPlaguicidaVUECompleto($conexion, $idProducto)
	{
	...
	public function obtenerUnidadesFitosanitarias($conexion, $idUnidad){
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT
											...
											WHERE
												id_unidad_fitosanitaria = '$idUnidad';");
	    
	    return $res;
	}
	
	public function obtenerCodigosComplementariosSuplementariosPlaguicidas($conexion, $idProducto, $partidaArancelariaVUE, $codigoProductoVUE){
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT
                                        	...
                                        	FROM
                                        	    ...
                                        	    g_catalogos.presentaciones_plaguicidas pp
                                        	WHERE
                                        	    pa.id_partida_arancelaria = ccs.id_partida_arancelaria
                                        	    AND pa.id_producto = '$idProducto'
                                        	    AND ccs.id_codigo_comp_supl = pp.id_codigo_comp_supl
                                        	    AND pa.partida_arancelaria = '$partidaArancelaria'
                                        	    AND pa.codigo_producto = '$codigo'
                                        	    AND ccs.codigo_complementario = '$codigoComplementario'
	...
	public function listarPaisesMasUnionEuropea($conexion){
				
		$res = $conexion->ejecutarConsulta("SELECT * 
											FROM g_catalogos.localizacion
											WHERE categoria = 0 or nombre = 'Unión Europea'
											ORDER BY 3;"); 													
		return $res;
	}
	
	public function reporteImprimirProductosInocuidadVeterinariaMag($conexion)
	{
		$res = $conexion->ejecutarConsulta("SELECT row_to_json(productosRPIV)
											FROM (SELECT
													pin.numero_registro,
	...
	public function reporteImprimirProductosInocuidadPlaguicidaMag($conexion)
	{
	    $res = $conexion->ejecutarConsulta("SELECT 
												row_to_json(productosRPIP)
											FROM (
	...
	public function destalleProductosInocuidad($conexion, $idProducto){
	    $cidProInocidad = $conexion->ejecutarConsulta("select distinct t.nombre tipo_producto,
														    p.id_producto,
															p.codigo_producto,
															p.partida_arancelaria subpartida,
	...
	public function obtenerProvinciaInspector($conexion, $identificadorInspector){  
	    $res = $conexion->ejecutarConsulta("SELECT
												*
											FROM
												g_uath.datos_contrato
											WHERE
												identificador = '$identificadorInspector';");
	    
	    return $res;
	}

	public function listarPuertos($conexion){
	...
	public function listarPuertosStodos($conexion){
		$cid = $this->listarPuertos($conexion);
		while ($fila = pg_fetch_assoc($cid)){
			$res[] = array(
				'id_puerto'=>$fila['id_puerto'],
				'nombre_puerto'=>$fila['nombre_puerto'],
				'id_pais'=>$fila['id_pais'],
				'codigo_puerto'=>$fila['codigo_puerto'],
