	...
	public function listarDetalleServicios($conexion,$servicio,$identificador,$provincia,$contador,$limit,$offset){
		...
		switch ($servicio){
			case 1:$consulta="SELECT 
    								...
    								ops.estado as estado,
    								(SELECT
									mias.nombre_miembro_asociacion || ' ' || mias.apellido_miembro_asociacion
									FROM g_operadores.miembros_asociacion AS mias
									INNER JOIN g_operadores.detalle_miembros_asociacion demias ON mias.id_miembro_asociacion = demias.id_miembro_asociacion 
									WHERE demias.id_operacion = ops.id_operacion) AS miembro,
		...
		while ($servicio = pg_fetch_assoc($consulta)) {
			...
			switch ($tipoServicio){
				case 'Operadores':
					...
					$columna2=$servicio['producto'];
					$columna3=$servicio['operacion'];
					$columna4=$servicio['estado'];
					$columna5=$servicio['miembro'];
