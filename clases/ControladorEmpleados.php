...
class ControladorEmpleados{
	...
	public function actualizarDatosPersonales($conexion, $identificador, $nombre, $apellido, $sexo, $estadoCivil, $nacimiento,$sangre,$nacionalidad,$etnia,$indigena,$domicilio,$convencional,$celular,$mailPersonal,$mailInstitucional,
	    ...
	    $provinciaNacimiento,$cantonNacimiento,$parroquiaNacimiento,$jornadaLaboral,$religion,$orientacionSexual,$lateralidad,$libretaMilitar,$telefonoInstitucional, $rutaPerfilPublico, $rutaQrPerfilPublico,$identidadgenero){
	...
	public function obtenerDatosCredencial($conexion,$identificador){
				
		$res = $conexion->ejecutarConsulta("
		SELECT
			CASE 
			WHEN position(' ' IN fic.nombre) > 0 AND position(' ' IN fic.apellido) > 0 THEN CONCAT(substring(fic.nombre, 1, (length(fic.nombre) - (position(' ' in reverse(fic.nombre))-1))), ' ', substring(fic.apellido, 1, (length(fic.apellido) - (position(' ' in reverse(fic.apellido))-1))))
			WHEN position(' ' IN fic.nombre) = 0 AND position(' ' IN fic.apellido) > 0 THEN CONCAT(fic.nombre, ' ', substring(fic.apellido, 1, (length(fic.apellido) - (position(' ' in reverse(fic.apellido))-1))))
			WHEN position(' ' IN fic.nombre) > 0 AND position(' ' IN fic.apellido) = 0 THEN CONCAT(substring(fic.nombre, 1, (length(fic.nombre) - (position(' ' in reverse(fic.nombre))-1))), ' ', fic.apellido)
			ELSE
			CONCAT(fic.nombre, ' ', fic.apellido)
			END as nombre,
			fic.identificador,
			fic.fotografia,
			con.direccion, 
			con.nombre_puesto
		FROM
			g_uath.ficha_empleado as fic,
			g_uath.datos_contrato as con
		WHERE
			fic.identificador='$identificador' and con.identificador = '$identificador' and con.estado = 1;");
