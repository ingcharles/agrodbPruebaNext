...
class ControladorVacaciones{
   ...
   public function obtenerDiaIngresoEmpleado($conexion, $dia){
    ...
    public function obtenerNombreDirector($conexion,$identificadorTH){
        ...
        $sqlScript="select quitar_caracteres_especiales_sin_espacio(dc.nombre_puesto) as puestoTH,
...
//--------------------------------------------------------------------------------------------------------
	...
	public function eliminarPermisoSinTiempo($conexion,$idPermiso, $observacion){
	    
	    $res = $conexion->ejecutarConsulta("update
													g_vacaciones.permiso_empleado
											set
													estado = 'eliminado',
                                                    observacion = 'Eliminado por usuario: ".$_SESSION['usuario']." con observación: ".$observacion."'
											where
													id_permiso_empleado = '".$idPermiso."';");
	    return $res;
	}
	
	public function eliminarPermisoConTiempo($conexion,$idPermiso, $observacion){
	...
	public function buscarRegistrosPermisoConTiempo($conexion,$idPermiso){
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT 
                                            ...
                                            FROM 
                                                g_vacaciones.detalle_descuento_tiempo
                                            WHERE
												id_permiso_empleado = '".$idPermiso."' and
                                                estado='creado';");
	    return $res;
	}
	
	public function buscarRegistrosTiempoUsuarioXAnio($conexion, $identificador, $anio){
	...
	public function actualizarTiempoDevolucionSaldo($conexion,$identificador, $anio, $tiempo, $observacion, $max){
	    
	    $res = $conexion->ejecutarConsulta("UPDATE 
                                            	g_vacaciones.minutos_disponibles_funcionarios
                                            SET 
                                            	minutos_disponibles='".$tiempo."', 
                                            	activo=true,
                                            	observacion='Actualizado por usuario: ".$_SESSION['usuario']." con observación: ".$observacion."'
                                            WHERE
                                            	identificador='".$identificador."' and
                                            	anio='".$anio."' and
                                                secuencial = '".$max."';");
	...
	public function actualizarTiempoDevolucionPermiso($conexion,$idDescuento){
	    ...
	    $res = $conexion->ejecutarConsulta("UPDATE 
                                            ...
                                            SET 
                                            	estado='devuelto'
                                            WHERE 
                                            	id_detalle_descuento=".$idDescuento.";");
	    return $res;
	}
	...
	public function listadoSaldoUsuarioAnual($conexion, $identificador, $estado, $apellido, $nombre){
	    
	    if($identificador != ''){
	        $busqueda = "mdf.identificador IN ('$identificador') and ";
	    }
	    
	    if($apellido != ''){
	        $busqueda .= " mdf.identificador IN (SELECT identificador FROM g_uath.ficha_empleado WHERE upper(apellido) like upper('%$apellido%')) and ";
	    }
	    
	    if($nombre != ''){
	        $busqueda .= " mdf.identificador IN (SELECT identificador FROM g_uath.ficha_empleado WHERE upper(nombre) like upper('%$nombre%')) and ";
	    }
	    
	    $res = $conexion->ejecutarConsulta("SELECT
                                                mdf.*, fe.nombre, fe.apellido
                                            FROM
                                                g_vacaciones.minutos_disponibles_funcionarios mdf,
                                                g_uath.ficha_empleado fe
                                            WHERE
                                                mdf.identificador = fe.identificador and
                                                ".$busqueda."
                                                mdf.activo = '$estado'
                                            ORDER BY 1,2");
	...
	public function saldoUsuarioAnual($conexion, $identificador, $estado, $anio){
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT
                                            ...
                                            WHERE
                                                mdf.identificador = fe.identificador and
                                                mdf.activo = '$estado' and
                                                mdf.identificador IN ('$identificador') and
                                                mdf.anio = '$anio'
                                            ORDER BY 1,2");
	...
	public function actualizarSaldoVacaciones ($conexion, $identificador, $anio, $tiempoNuevo, $secuencial, $observacion){
	    
	    $res = $conexion->ejecutarConsulta("UPDATE 
                                                g_vacaciones.minutos_disponibles_funcionarios
                                        	SET 
                                                minutos_disponibles='$tiempoNuevo', 
                                                observacion='Actualizado por: ".$_SESSION['usuario']." con observación: ".$observacion."'
                                        	WHERE 
                                                identificador='$identificador' and 
                                                anio='$anio' and 
	...
	public function obtenerDirectorXArea($conexion,$idArea){
        
        $res = $conexion->ejecutarConsulta("select quitar_parentesis(quitar_caracteres_especiales_sin_espacio(dc.nombre_puesto)) as puestoresponsable,
												   fe.nombre || ' ' || fe.apellido as nombreresponsable,
												   f.id_area,
												   r.id_area as area_responsable
											from
													g_uath.ficha_empleado fe, g_uath.datos_contrato dc, g_estructura.funcionarios f, g_estructura.responsables r
											where
													dc.identificador=fe.identificador and
													dc.identificador=f.identificador and
													dc.identificador=r.identificador and
													r.id_area='$idArea' and
													r.estado=1 and
													dc.estado=1;");
    ...
    public function obtenerResponsableTTHHXArea($conexion,$idArea){
        
        $res = $conexion->ejecutarConsulta("SELECT
                                				quitar_caracteres_especiales_sin_espacio(dc.nombre_puesto) as puestoresponsable,
												fe.nombre || ' ' || fe.apellido as nombreresponsable
                                			FROM
                                				g_uath.encargo_recursos_humanos rh
                                				INNER JOIN g_uath.ficha_empleado fe ON rh.identificador = fe.identificador
												INNER JOIN g_uath.datos_contrato dc ON dc.identificador = fe.identificador
                                			where
                                				rh.zona_area = '$idArea' and
