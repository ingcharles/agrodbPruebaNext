...
class ControladorDocumentos{
	...
	function generarNombreArchivo($conexion,$idDocumento){
		...
		$res = $conexion->ejecutarConsulta("select
                                            ...
                                            where
                                            	...
                                            	dg.version_plantilla=".$version." and
                                            	dg.codigo_plantilla=".$plantilla." and
                                            	id_documento like '".$area."-".$anio."%".$numeroPlantilla.$numeroVersion."."."%'");
		
		
		$numeracion= pg_fetch_result($res,0,'numero');
		
		$listado[] = null;
		
		if(strlen($numeracion) > 0){

		    while ($num = pg_fetch_assoc($res)){
		        $stringSeparado = explode('.', $num['numero']);
		        array_push ($listado,(int)$stringSeparado[1]);
		    }
		    
		    rsort($listado);
		    
		    $numero = (int)$listado[0] + 1;
		    $codigoFinal = str_pad($numero, 4, "0", STR_PAD_LEFT);
		    $incremental = $codigoFinal;		    
		    
		}else{
		    $incremental = str_pad(1, 4, "0", STR_PAD_LEFT);
		}
	...
	public function eliminarDocumento($conexion,$idDocumento, $observacion){
	    
	    $res = $conexion->ejecutarConsulta("update
													g_documentacion.documentos_generados
											set
													estado = 9,
                                                    estado_mail = '".$observacion."'
											where
													id_documento = '".$idDocumento."';");
	    return $res;
	}
