...
class ControladorFinancieroAutomatico{
	...
	public function obtenerDatosGenerarFacturaAutomatico($conexion, $estado){
		...
		$res = $conexion->ejecutarConsulta("SELECT
											...
											WHERE
												estado_factura = '$estado'
												and tipo_solicitud NOT IN ('certificadoFito');");
	...
	public function obtenerDatosGenerarFacturaAutomaticoSolicitudes($conexion, $estado){
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT
											...
											WHERE
												estado_factura $estado
	                                            and tipo_solicitud in ('certificadoFito');");
	    
	    return $res;
	    
	}

	public function guardarFinancieroAutomaticoCabeceraSolicitudes ($conexion, $totalPagar, $idVue, $tipoSolicitud, $metodoPago = null){
	    
	    $consulta = "INSERT INTO
                        g_financiero_automatico.financiero_cabecera
                        (total_pagar, id_vue, tipo_solicitud, metodo_pago)
                    SELECT " . $totalPagar . ", '" . $idVue . "', '" . $tipoSolicitud . "', '" . $metodoPago . "'
                    WHERE NOT EXISTS
    					(SELECT
                                1
                            ...
                            ) RETURNING id_financiero_cabecera;";
