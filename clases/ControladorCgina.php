...
class ControladorCgina
{
	...
	public function obtenerCentrosAcopioLeche($conexion,$fecha){
		...
		$res = $conexion->ejecutarConsulta("SELECT
										...
										WHERE
											".$condicion."
											and CASE WHEN opc.estado = 'registrado' THEN opc.fecha_aprobacion WHEN opc.estado = 'porCaducar' OR opc.estado = 'noHabilitado' THEN opc.fecha_modificacion ELSE opc.fecha_creacion END >= '$fecha'
    ...
    public function insertarCentrosAcopioLeche($conexion,$valores,$tabla){
        $consulta="INSERT INTO 
                    mag.$tabla(identificador, codigo_operador_leche, nombre_producto, fecha_creacion, 
                    fecha_aprobacion, 
                    fecha_finalizacion, observacion, provincia, canton, 
                    parroquia, nombre_area, direccion, nombre_operador, 
                    operacion, numero_provedores,hora_recoleccion_maniana,hora_recoleccion_tarde,estado)
                VALUES 
                    $valores;";
        $res = $conexion->ejecutarConsulta($consulta);
        return $res;
