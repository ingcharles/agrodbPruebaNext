...
public function listarRecaudacionExcedentes($conexion, $tipo, $fechaInicio, $fechaFin, $localidad, $valor, $cliente, $ruc, $tipoSaldo='saldoAgr'){
	...
	public function listarReporteRecargaSaldosPorNumeroEstablecimiento($conexion, $comprobante, $fechaInicio, $fechaFin, $provincia, $establecimiento, $ruc){
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT
                                                ...
                                                , op.numero_solicitud
                                                , op.observacion
                                                , op.numero_establecimiento || '-' || op.punto_emision || '-' ||op.numero_factura as numero_comprobante
                                                , to_char(op.fecha_facturacion,'YYYY-MM-DD') as fecha_facturacion
                                                , op.numero_establecimiento as provincia_numero_establecimiento
                                                , s.codigo
                                                , s.concepto
                                                , dp.cantidad
                                                , dp.precio_unitario
                                                ...
                                                , dp.total
                                                , dp.subsidio
                                                , dfp.institucion_bancaria
                                                , dfp.transaccion
                                                , dfp.valor_deposito
                                                , to_char(dfp.fecha_orden_pago,'YYYY-MM-DD') as fecha_deposito
                                             FROM
                                                g_financiero.orden_pago op
                                                INNER JOIN g_financiero.clientes c ON c.identificador = op.identificador_operador
                                                INNER JOIN g_financiero.detalle_pago dp ON dp.id_pago = op.id_pago
                                             ...
                                             WHERE
                                                op.tipo_solicitud = '" . $comprobante ."'
                                                and op.fecha_facturacion >= '" . $fechaInicio . " 00:00:00'
                                                and op.fecha_facturacion <= '" . $fechaFin . " 24:00:00'
                                                and ($ruc is NULL or op.ruc_institucion = $ruc)
                                                and ($establecimiento is NULL or op.numero_establecimiento = $establecimiento)
                                                and op.estado = 4
                                                ORDER BY op.fecha_facturacion DESC;");
	    
	    return $res;
	}
	
	public function listarReporteFacturasAnuladasPorNumeroEstablecimiento($conexion, $fechaInicio, $fechaFin, $provincia, $establecimiento, $ruc){
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT 
                                            	...
                                            	, op.fecha_facturacion
                                            	, op.estado_sri
                                            	, taf.motivo_anulacion
                                            	, op.tipo_solicitud
                                            	, op.nombre_provincia
                                            	, taf.identificador_responsable_anulacion 
                                            	, taf.nombre_responsable_anulacion
                                            	, taf.fecha_anulacion_factura
                                            FROM 
                                            	g_financiero.orden_pago op 
                                            INNER JOIN g_financiero.clientes c ON op.identificador_operador = c.identificador
                                            LEFT JOIN (SELECT
                                            				af.id_pago
                                            				, af.identificador_usuario AS identificador_responsable_anulacion
                                            				, fe.nombre || ' ' || fe.apellido AS nombre_responsable_anulacion
                                            				, af.motivo_anulacion
                                            				, af.fecha_anulacion_factura
                                            			FROM
                                            				g_financiero.anulacion_factura af 
                                            			INNER JOIN g_uath.ficha_empleado fe ON fe.identificador = af.identificador_usuario) taf ON taf.id_pago = op.id_pago
                                            WHERE
                                        	    op.fecha_autorizacion >= '" . $fechaInicio . " 00:00:00'
                                    	        and op.fecha_autorizacion <= '" . $fechaFin . " 24:00:00'
                            	                and ($ruc is NULL or op.ruc_institucion = $ruc)
                                                and ($establecimiento is NULL or op.numero_establecimiento = $establecimiento)
                        	                    and op.estado = 9
                                            	and op.estado_sri = 'ANULADO'
                        	                 ORDER BY op.nombre_provincia, taf.fecha_anulacion_factura ASC;");
