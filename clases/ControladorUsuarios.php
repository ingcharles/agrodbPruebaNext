...
class ControladorUsuarios{
	...
	public function verificarUsuario($conexion, $identificador, $perfil=null){
		...
		if($perfil) {
			...
			$consulta = "select
							  u.clave, u.nombre_usuario, u.estado, u.codigo_temporal, u.aceptar_politica
						 from
							    g_usuario.usuarios u
						  where
	...
	*/
		...
		try {
			...
			if($tipo == "Interno"){
				...
				$res = $conexion->ejecutarConsultaLOGS("INSERT INTO
						...
						VALUES ('$numero',2),('$numero',73),('$numero',245),('$numero',243);");
				...
				$res = $conexion->ejecutarConsultaLOGS("INSERT INTO
						...
						(78,'$numero',0,'notificaciones'),
						(77,'$numero',0,'notificaciones')
	...
	public function obtenerDatosEmpleado($conexion, $identificador) {
	    ...
	    $consulta = "SELECT
						...
						dc.id_gestion AS id_gestion,
                        fe.nombre || ' ' || fe.apellido as dato_usuario
					FROM
						g_uath.datos_contrato dc
	...
	public function crearUsuarioBanano($conexion, $ruc, $clave){
		
		$res = $conexion->ejecutarConsulta("INSERT INTO 
									g_usuario.usuarios (identificador,nombre_usuario,clave,estado )
									SELECT  '$ruc','$ruc','$clave', 3
									WHERE NOT EXISTS (
										SELECT identificador FROM g_usuario.usuarios WHERE identificador = '$ruc');");
		
		return $res;
	}
	
	public function actualizarPoliticaProteccionDatos($conexion, $aceptarPolitica, $identificador){
	    
	    $consulta ="UPDATE
						g_usuario.usuarios
					SET
						aceptar_politica = true
                        , fecha_aceptar_politica = now()
					WHERE
						identificador = '" . $identificador . "'";
