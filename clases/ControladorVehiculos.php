...
class ControladorVehiculos{
	...
	public function datosVehiculos ($conexion, $localizacion){
		$res = $conexion->ejecutarConsulta("(select  
                                                    ...
                                                    ve.localizacion,
													'LOCAL' as tipo,
                                                    (select m.fecha_fin from g_transportes.matriculaciones m
                                                        where m.placa=ve.placa and
                                                        m.tipo_documento='Matrícula del Vehículo' and
                                                        m.estado_documento not in ('Eliminado')
    												 	order by m.fecha_fin desc
                                                        limit 1) as vigencia_matricula,
    												(select m.fecha_fin from g_transportes.matriculaciones m
                                                        where m.placa=ve.placa and
                                                        m.tipo_documento='Certificado de Matriculación Anual' and
                                                        m.estado_documento not in ('Eliminado')
    												 	order by m.fecha_fin desc
                                                    	limit 1) as vigencia_certificado_matricula
												from	g_transportes.vehiculos ve,
													g_estructura.area a,
													g_usuario.usuarios u,
													g_uath.ficha_empleado fe
	...
	public function actualizarEstadoVehiculo($conexion, $placa, $condicion){
		...
		switch ($condicion){
			...
			case 'Matriculacion': $actualiza = "estado = 5 "; break;
	...
	$res = $conexion->ejecutarConsulta("UPDATE
										...
										SET
											...
											identificador_registro = '$identificadorUsuario'
										WHERE
...
public function abrirMantenimientoDetalle ($conexion, $id_mantenimiento){
    ...
    //MSD
    //Eliminación órdenes
	public function darBajaMovilizacionAdm ($conexion, $idMovilizacion, $usuarioSolicitante, $glpi, $identificadorUsuarioRegistro){
	    
	    $res = $conexion->ejecutarConsulta("UPDATE
												g_transportes.movilizaciones
											SET
												estado = 9,
												observacion_movilizacion = 'Eliminada por solicitud de $usuarioSolicitante en ticket $glpi',
												identificador_registro = '$identificadorUsuarioRegistro'
											WHERE
												id_movilizacion = '$idMovilizacion' ;");
	    return $res;
	...
	public function darBajaMantenimientoAdm ($conexion, $idMantenimiento, $usuarioSolicitante, $glpi, $identificadorUsuarioRegistro){
	...
	//Matriculación
	public function datosVehiculosMatriculacion ($conexion, $localizacion){
	    ...
	    $res = $conexion->ejecutarConsulta("select
												...
												(select m.fecha_fin from g_transportes.matriculaciones m
                                                    where m.placa=ve.placa and
                                                    m.tipo_documento='Matrícula del Vehículo' and
                                                    m.estado_documento in ('Vigente', 'Por caducar')
												 	order by m.fecha_fin desc
                                                    limit 1) as vigencia_matricula,
												(select m.estado_documento from g_transportes.matriculaciones m
                                                    where m.placa=ve.placa and
                                                    m.tipo_documento='Matrícula del Vehículo' 
												 	order by m.fecha_fin desc
                                                    limit 1) as estado_matricula,	
												(select m.fecha_fin from g_transportes.matriculaciones m
                                                    where m.placa=ve.placa and
                                                    m.tipo_documento='Certificado de Matriculación Anual' and
                                                    m.estado_documento in ('Vigente', 'Por caducar')
											...
											from	
                                                g_transportes.vehiculos ve
											where
												ve.localizacion = '$localizacion'
												and ve.estado not in (9);");
	    return $res;
	}
	
	public function obtenerMatriculacionXVehiculo ($conexion, $placa){
	    
	    $res = $conexion->ejecutarConsulta("select
												* 
                                            from 
                                                g_transportes.matriculaciones m
                                            where 
                                                m.placa = '$placa' and
                                                m.estado_documento not in ('Eliminado')
											order by 
                                                m.tipo_documento, m.fecha_fin desc;");
	    return $res;
	}
	
	public function buscarMatriculaXVechiculoXFecha($conexion, $placa, $tipo, $fechaInicio, $fechaFin){
	    
	    $res = $conexion->ejecutarConsulta("SELECT
												*
											FROM
												g_transportes.matriculaciones
											WHERE
												placa = '$placa' and
                                                tipo_documento = '$tipo' and
	...
	public function guardarNuevaMatricula ($conexion,$placa, $tipoDocumento, $fechaInicio, $fechaFin, $rutaArchivo, $identificador){
	    $res = $conexion->ejecutarConsulta("INSERT INTO
												g_transportes.matriculaciones (placa, tipo_documento, fecha_inicio, fecha_fin, ruta_documento, identificador)
											VALUES
												('$placa', '$tipoDocumento', '$fechaInicio', '$fechaFin', '$rutaArchivo', '$identificador')
											RETURNING id_matriculacion;");
	    return $res;
	}
	
	public function imprimirLineaMatricula($idMatriculacion, $placa, $tipoDocumento, $fechaInicio, $fechaFin, $rutaArchivo, $ruta, $estado, $tipoUsuario=null){
	    
	    if($tipoUsuario == 'Administrador'){
	        $opcionEliminar =   '<td width="10%">' .
                    	   	        '<form class="borrar" data-rutaAplicacion="'.$ruta.'" data-opcion="eliminarMatriculacion">' .
                    	   	        '<input type="hidden" name="idMatriculacion" value="' . $idMatriculacion . '" >' .
	    ...
	    return '<tr id="R' . $idMatriculacion . '">' .
	   	    '<td width="10%">' .
	   	    $placa .
	   	    '</td>' .
	   	    '<td width="20%">' .
	   	    ...
	   	    '</td>' .
	   	    '<td width="10%">
	   	    <a href="'.$rutaArchivo.'" target="_blank">Enlace</a>'.
	   	    '</td>' .
	   	    '<td width="10%">' .
	   	    $estado .
	   	    '</td>' .
	...
	public function actualizarEstadoMatriculacionesIngreso($conexion, $placa, $tipo, $idMatriculacion, $identificador){
	    
	    $res = $conexion->ejecutarConsulta("UPDATE
                                                g_transportes.matriculaciones
                                        	SET
                                                estado_documento='Inactivo',
                                                observacion='Actualizado por nuevo ingreso manual de registro $idMatriculacion por usuario $identificador',
                                                identificador_modificacion = '$identificador',
                                                fecha_modificacion = now()
                                        	WHERE
                                                placa = '$placa' and
                                            	tipo_documento = '$tipo' and
                                            	id_matriculacion not in ($idMatriculacion) and
                                                estado_documento not in ('Eliminado');");
	    return $res;
	}
	
	//Administración
	public function datosVehiculosMatriculacionNacional ($conexion, $placa, $provincia, $oficina){
	    
	    if(isset($placa) && ($placa != null && $placa != '')){
	        
	        $query = " and ve.placa = '$placa' ";
	        
	    }

	    if(isset($provincia) && ($provincia != null && $provincia != '')){
	        
	        $query = " and ve.localizacion in (select
                            						distinct oficina
                            					from
	    ...
	    if(isset($oficina) && ($oficina != null && $oficina != '')){
	        
	        $query = " and ve.localizacion = '$oficina' ";
	    ...
	    $res = $conexion->ejecutarConsulta("select
												...
												(select m.fecha_fin from g_transportes.matriculaciones m
                                                    where m.placa=ve.placa and
                                                    m.tipo_documento='Certificado de Matriculación Anual'
												 	order by m.fecha_fin desc
                                                	limit 1) as vigencia_certificado_matricula
											from
                                                g_transportes.vehiculos ve
	...
	public function darBajaMatriculacion ($conexion, $idMatriculacion, $identificador){
	    
	    $res = $conexion->ejecutarConsulta("UPDATE
												g_transportes.matriculaciones
											SET
												estado_documento = 'Eliminado',
												observacion = 'Eliminado por usuario $identificador',
                                                fecha_modificacion = now(),
												identificador_modificacion = '$identificador'
											WHERE
												id_matriculacion = '$idMatriculacion';");
	    return $res;
	}
	
	public function validarMatriculasActivas ($conexion, $placa){
	    
	    $res = $conexion->ejecutarConsulta("select
												distinct m.estado_documento 
                                            from 
                                                g_transportes.matriculaciones m
                                            where 
                                                m.placa = '$placa' and
                                                m.estado_documento not in ('Eliminado', 'Inactivo', 'Caducado');");
                                                 
        return $res;
	}
	
	public function obtenerMatriculacionXVehiculoXTipo ($conexion, $placa, $tipo){
	...
	public function actualizarMatriculacionLiberacion($conexion, $idMatriculacion, $motivo){
	    ...
	    $res = $conexion->ejecutarConsulta("UPDATE
											...
											SET
                                                ...
                                                bandera_liberacion = 'Liberado'
											WHERE
												id_matriculacion in ($idMatriculacion);");
	    
	    return $res;
	}
	
	public function obtenerReporteMatriculacionesGeneradas($conexion,$tipo, $localizacion, $fechaInicio, $fechaFin){
	    
	    $query = "";
	    
	    if($tipo == 'Administrador'){
	        if($localizacion != 'Nacional'){
	            $query = " WHERE v.localizacion in (
                            select
        						distinct oficina
        					from
        						g_uath.datos_contrato
        					where
        						provincia='$localizacion' and
    							estado = 1
        					order by
        						oficina asc)
                            ";
	        ...
	        $query = " WHERE v.localizacion = '$localizacion' ";
	    }
	    
	    if(isset($fechaInicio) && $fechaInicio != null && $fechaInicio != ''){
	        $query .= " and m.fecha_inicio >= '$fechaInicio' ";
	    }

	    if(isset($fechaFin) && $fechaFin != null && $fechaFin != ''){
	        $query .= " and m.fecha_fin <= '$fechaFin' ";
	    }
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT
                                                ...
                                                (
                                                    ...
                                                    from g_uath.ficha_empleado where    
                                                    identificador = v.identificador
                                                ) as administrador_vehiculo,
                                                CASE 
                                                    WHEN v.estado=1 THEN 'Libre'
                                            		WHEN v.estado=2 THEN 'Mantenimiento'
                                            		WHEN v.estado=3 THEN 'Movilización'
                                                    WHEN v.estado=4 THEN 'Siniestro'
                                                    WHEN v.estado=5 THEN 'Matriculación'
                                                    WHEN v.estado=9 THEN 'Eliminado'
                                                END as estado,
                                                m.*
    										FROM
    											g_transportes.matriculaciones m
                                                INNER JOIN g_transportes.vehiculos v ON m.placa= v.placa
                                                $query											
    										ORDER BY
    											m.placa, m.tipo_documento, m.fecha_inicio asc;");
	    
	    return $res;
	}
	
	//Proceso Automático
	public function buscarMatriculasCaducadas($conexion){
	    ...
	    $consulta = "   SELECT
                        ...
                        WHERE
                        	fecha_fin <= now() and
							estado_documento not in ('Eliminado','Inactivo');" ;
	...
	public function cambioEstadoMatriculacionCaducado($conexion, $idMatriculacion){
	    
	    $consulta = "   UPDATE
                        	g_transportes.matriculaciones
                        SET
                        	estado_documento = 'Caducado',
                        	observacion = 'Cambio de estado a Caducado por proceso automático',
                        	ejecucion_trigger = 'now()'
                        WHERE
                        	--estado_documento = 'Por Caducar' and
                        	id_matriculacion= $idMatriculacion and
                            estado_documento not in ('Eliminado', 'Inactivo');" ;
	...
	public function buscarCertificadoMatriculasXCaducar($conexion){
	    ...
	    $consulta = "   SELECT
                        ...
                        FROM
                        	g_transportes.matriculaciones
                        WHERE
                            tipo_documento = 'Certificado de Matriculación Anual' and 
                        	fecha_fin - interval '6 months' <= now() and 
                            estado_documento not in ('Eliminado', 'Inactivo');" ;
	...
	public function buscarMatriculasXCaducar($conexion){
	    ...
	    $consulta = "   SELECT
                        ...
                        FROM
                        	g_transportes.matriculaciones
                        WHERE
                            tipo_documento = 'Matrícula del Vehículo' and
                        	extract(year from fecha_fin) <= extract(year from now()) and 
                            estado_documento not in ('Eliminado', 'Inactivo');" ;
	...
	public function cambioEstadoMatriculacionXCaducar($conexion, $idMatriculacion){
	    
	    $consulta = "   UPDATE
                        	g_transportes.matriculaciones
                        SET
                        	estado_documento = 'Por Caducar',
                        	observacion = 'Cambio de estado a Por Caducar por proceso automático',
                        	ejecucion_trigger = 'now()'
                        WHERE
                        	--estado_documento = 'Vigente' and
                        	id_matriculacion= $idMatriculacion and 
                            estado_documento not in ('Eliminado', 'Inactivo');" ;
	...
	//Administración de usuarios provincia
	public function datosAdministradoresNacional ($conexion, $provincia, $oficina){
	    
	    if(isset($provincia) && ($provincia != null && $provincia != '')){
	        
	        $query = " dc.provincia = '$provincia' and ";
	        
	    }
	    
	    if(isset($oficina) && ($oficina != null && $oficina != '')){
	        
	        $query = " dc.oficina = '$oficina' and ";
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT
                                            ...
                                            FROM	
                                            	...
                                            	INNER JOIN g_usuario.usuarios_perfiles up ON dc.identificador = up.identificador
                                                INNER JOIN g_uath.ficha_empleado fe ON fe.identificador = dc.identificador
                                            WHERE
                                            	dc.estado = 1 and
                                            	$query
                                            	up.id_perfil = 19
                                             ORDER BY provincia, oficina;");
                                                 
         return $res;
	}
	
	public function buscarAdministradoresXProvinciaXOficina($conexion, $provincia, $oficina){
	    ...
	    $consulta = "   SELECT
                        ...
                        WHERE
                        	dc.estado = 1 and
                        	dc.provincia = '$provincia' and
                        	dc.oficina = '$oficina' and
                        	up.id_perfil = 19;" ;
	...
	public function buscarAdministradorXProvinciaXOficina($conexion, $identificador, $provincia, $oficina){
	...
	public function buscarDatosAdministrador ($conexion, $identificador){
	...
	//AUDITORIA
	public function guardarAuditoriaAdministrador($conexion, $identificador, $identificadorAdministrador, $provincia, $oficina, $accion, $observaciones){
	    
	    $res = $conexion->ejecutarConsulta("INSERT into g_transportes.auditoria_administradores(
												identificador, identificador_administrador, provincia, oficina, accion, observaciones)
											VALUES ('$identificador','$identificadorAdministrador', '$provincia', '$oficina', '$accion', '$observaciones');");
	    return $res;
	}
}