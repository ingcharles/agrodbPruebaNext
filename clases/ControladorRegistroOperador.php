...
class ControladorRegistroOperador{
	...
	public function actualizarSitio ($conexion,$codigoCatastral, $nombreSitio,$superficie,$provincia,$canton,$parroquia, $direccion, $referencia, $latitud, $longitud, $zona, $telefono, $idSitio, $identificador, $codigoProvincia, $codigoSitio){
		...
		$res = $conexion->ejecutarConsulta("UPDATE 
											...
											SET
												...
												codigo_catastral = '$codigoCatastral',
												latitud = '$latitud',
												longitud = '$longitud',
												zona = '$zona',
	...
	public function guardarNuevoSitio ($conexion,$codigoCatastral,$nombreSitio,$provincia,$canton,$parroquia,$direccion,$referencia,$superficie,$identificador,$telefono,$latitud, $longitud, $codigoSitio, $croquis, $zona){
	
		$res = $conexion->ejecutarConsulta("INSERT INTO g_operadores.sitios(
            												nombre_lugar, provincia, canton, parroquia, direccion, referencia, superficie_total, identificador_operador, estado, telefono,latitud, longitud, codigo, croquis, zona, codigo_provincia, codigo_catastral)
    										VALUES ('$nombreSitio','$provincia','$canton','$parroquia','$direccion', '$referencia', $superficie,'$identificador','creado','$telefono','$latitud', '$longitud', '$codigoSitio', '$croquis', '$zona' , '$codigoProvincia', $codigoCatastral) RETURNING id_sitio;");
				return $res;
	}
		...
		$cid = $conexion->ejecutarConsulta("select 
			...
			while ($fila = pg_fetch_assoc($cid)){
				$res[] = array('idSolicitud'=>$fila['id_operacion'],'idTipoOperacion'=>$fila['id_tipo_operacion'],'tipoOperacion'=>$fila['nombre'],
            				    ...
            				    'codigoProvincia'=>$fila['codigo_provincia'], 'codigo_catastral'=>$fila['codigo_catastral']
				);
			}
...
/*public function jsonBuscarOperacionesPorProvincia($conexion, $provincia, $area, $canton){
													...
													from (
														...
														from (
                                                            ...
                                                            where
																...
																opr.id_tipo_operacion not in (28,29,30,31,32,33,38,39,132,133)
														...
														from (
														    select
                                                                ...
                                                                where
																	...
																	o.id_tipo_operacion not in (28,29,30,31,32,33,38,39,132,133)
														...
														from (
														    select
                                                                ...
                                                                where
        															...
        															and o.id_tipo_operacion not in (28,29,30,31,32,33,38,39,132,133)
																	and o.estado in ('registrado','registradoObservacion','inspeccion')
                                                                order by
                                                                    1
												) as listado)
 ...
 public function obtenerDatosPorOperacion($conexion, $identificador, $tipo_operacion,$area) {
 	...
 	set_time_limit (120);
        ...
        public function listarOperacionesQueRequierenAnexos($conexion, $usuario, $filtro) {
        	$res = $conexion->ejecutarConsulta("select
											...
											where " . $filtro . 
        										...
        										topr.estado = 'activo'
												and o.estado IN ('cargarAdjunto','subsanacion')
       ...
       public function listarProductosXOperacionXAreas ($conexion,$idArea,$idOperacion){
		...
		public function imprimirLineaProductoOperacion($idSolicitud, $nombreTipoProducto, $nombreSubtipoProducto, $nombreProducto, $idProduto, $validacionProducto, $procesoEliminacion, $idOperacion = null){
			...
			$cadena = '<tr id="R'.$idSolicitud.'">' .
					...
					'<td  width="25%" style="text-align: center;"><input type="checkbox" name="agencia[]" value="'.$idOperacion.'"></td>';
		...
		public function obtenerProductosOrganicosPorIdOperadorTipoOperacionHistorico($conexion, $idOperadorTipoOperacion, $idHistorialOperacion, $estado){
			...
			$res = $conexion->ejecutarConsulta("SELECT 
												...
												INNER JOIN g_operadores.miembros_asociacion mias ON mias.id_miembro_asociacion = dma.id_miembro_asociacion
												WHERE 
													o.id_operador_tipo_operacion = $idOperadorTipoOperacion 
													and o.id_historial_operacion = $idHistorialOperacion 
													and o.estado = '$estado' 
	...
	public function obtenerDatosOperadorLecheXProviciaXFecha($conexion, $idProvincia, $fechaInicio, $fechaFin, $tipoOperacion){
		...
		switch($tipoOperacion){
			...
			break;
			case 'AI-MDT';
				$columnas = ", dv.placa_vehiculo, dv.nombre_tipo_vehiculo as tipo_vehiculo , dv.nombre_marca_vehiculo as marca_vehiculo, dv.nombre_color_vehiculo as color_vehiulo, dv.anio_vehiculo, ttv.nombre as tipo_tanque_vehiculo, dv.capacidad_vehiculo, dv.hora_inicio_recoleccion, dv.hora_fin_recoleccion, opc.observacion";
				$tablas = " LEFT JOIN (SELECT 
											dv1.id_tipo_tanque_vehiculo
										   , dv1.nombre_color_vehiculo
										   , dv1.nombre_marca_vehiculo
										   , dv1.nombre_tipo_vehiculo
										   , dv1.placa_vehiculo
										   , dv1.anio_vehiculo
										   , dv1.capacidad_vehiculo
										   , dv1.hora_inicio_recoleccion
										   , dv1.hora_fin_recoleccion
										   , dv1.id_operador_tipo_operacion
										   , dv1.id_area 
										FROM
	...
	public function obtenerOperacionesBanano($conexion, $estado = 'Por atender'){
	    ...
	    $consulta = "SELECT * FROM public.operaciones_banano WHERE estado in ('".$estado."') ;";
	...
	public function obtenerOperacionesBananoPorArea($conexion){
	    
	    $consulta = "SELECT * FROM public.operaciones_banano WHERE estado in ('Atendida') ;";
	...
	public function obtenerProveedoresBanano($conexion, $estado = 'Por atender',$limit){
		
		$consulta = "SELECT * FROM public.proveedores_banano WHERE estado_contrato = '$estado'".$limit.";";
	...
	public function bucarAreaPorNombreSitioTipoArea($conexion, $idSitio, $nombre, $tipoArea, $codigoTransaccional){
	    
	    $consulta = "SELECT * FROM g_operadores.areas WHERE nombre_area = '$nombre' 
                     and id_sitio = '$idSitio' and tipo_area ilike ('%$tipoArea%')  and codigo_transaccional = '$codigoTransaccional';";
	 ...
	 */
	 public function obtenerFechaCreacionRegistroOrganicos($conexion, $idOperador, $idTipoOperacion){
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT
												to_char(coalesce(min(fecha_creacion), min(fecha_aprobacion)),'YYYY-MM-DD') as fecha_creacion
											FROM
												g_operadores.operaciones
											WHERE
												identificador_operador = '" . $idOperador . "'
	...
	public function obtenerFechaActualizacionRegistroOrganicos($conexion, $idOperador, $idTipoOperacion){
	    ...
	    $res = $conexion->ejecutarConsulta("SELECT
												to_char(coalesce(max(fecha_modificacion), max(fecha_aprobacion)),'YYYY-MM-DD') as fecha_actualizacion
											FROM
												g_operadores.operaciones
											WHERE
												identificador_operador = '" . $idOperador . "'
	...
	public function buscarAreaBanano($conexion,$arrayParametros){
	    
		$consulta = "SELECT  sit.id_sitio, ar.id_area,ar.codigo_transaccional, sit.identificador_operador from g_operadores.areas ar
						inner join g_operadores.sitios sit
						on ar.id_sitio = sit.id_sitio					
						where ar.codigo_transaccional is not null and ar.codigo_transaccional<>'' and ar.codigo_transaccional = '".$arrayParametros['codigo_hacienda']."' 
						and identificador_operador = '".$arrayParametros['identificador']."' and tipo_area  ilike ('%".$arrayParametros['tipo_area']."%')";
	...
	public function bucarSitioPorNombreIdentificadorAuditoria($conexion, $identificador,$id,$codigoTransaccion ,$estado){
	    
		$consulta = "SELECT * FROM public.auditoria_operaciones_banano 
		WHERE identificador = '$identificador' and id=$id   and codigo_hacienda = '$codigoTransaccion';";
	...
	public function actualizarEstadoAuditoriaOperacionesBanano($conexion, $identificador,$id,$estado){
	    
		$consulta = "UPDATE public.auditoria_operaciones_banano SET estado_registro = '$estado' 
		WHERE identificador = '".$identificador."' and id = ".$id." ;";
	...
	public function obtenerMercadosDestino($conexion, $idOperadorTipoOperacion, $idOperador){
		$consulta = "SELECT distinct l.nombre as mercado_destino
						FROM
						g_operadores.operadores o
						INNER JOIN g_operadores.operaciones op ON o.identificador = op.identificador_operador
						INNER JOIN g_catalogos.tipos_operacion top ON op.id_tipo_operacion = top.id_tipo_operacion
						...
						WHERE
						op.identificador_operador = '" . $idOperador . "'
						and op.estado='registrado'
						and op.id_tipo_operacion = '" . $idOperadorTipoOperacion . "';";

		$unionEuropea = false;
	    $res = $conexion->ejecutarConsulta($consulta);

	    while($pais = pg_fetch_assoc($res)){
	    	if(trim($pais['mercado_destino']) == 'Unión Europea'){
	    		$unionEuropea = true;
	    	}
	    }
	    if($unionEuropea){
	    	$consulta = "SELECT distinct l.nombre as mercado_destino
						...
						and op.estado='registrado'
						and op.id_tipo_operacion = '" . $idOperadorTipoOperacion . "'
						and l.id_localizacion_padre IS NULL;";
			$res = $conexion->ejecutarConsulta($consulta);
	    }
	...
	//Cambios alejandro camacho
	public function listarSitiosFiltrados ($conexion,$identificador, $filtro){
	
		$res = $conexion->ejecutarConsulta("select
													s.*, (select count(a.id_sitio) from g_operadores.areas a where a.id_sitio = s.id_sitio) as num_areas, s.identificador_operador || '.' || s.codigo_provincia || s.codigo as codigo_sitio
											from
													g_operadores.sitios s
											where 	" . $filtro .  "
													s.identificador_operador = '$identificador'
	...
	public function listarSitiosFiltradosCombos ($conexion,$identificador,$tipo){
		...
		$res = $conexion->ejecutarConsulta("select
											...
											where 
													s.identificador_operador = '$identificador'
													and s.estado not in ('eliminado','inactivo') GROUP BY " . $tipo . " ORDER BY " . $tipo . " ASC;");
		return $res;
	}

	public function listarOperacionesOperadorFiltrados($conexion,$identificador, $estado, $incremento, $datoIncremento, $tipoEstado = null, $filtro){
		    $busqueda = '';
		    if($tipoEstado == 'cargarProducto'){
		        $busqueda = "and t.codigo NOT IN ('IMP','EXP')";
		    }else if($tipoEstado == 'representanteTecnico'){
		    ...
		    $res = $conexion->ejecutarConsulta("select
												...
												where ".$filtro."
													...
													t.id_flujo_operacion = fo.id_flujo
			                                        ".$busqueda."
												group by o.razon_social, s.identificador_operador, s.estado, s.id_tipo_operacion, nombre_tipo_operacion, st.provincia, st.id_sitio, a.id_area, t.codigo
												order by id_operacion;");
		    return $res;
		}
		
		public function listarOperacionesOperadorFiltradosCombos ($conexion,$identificador, $estado, $tipo){
		...
		public function listarDocumentosAnexosFiltro($conexion, $usuario, $filtro) {
        	$res = $conexion->ejecutarConsulta("
								        		select
								        			oda.id_documento_anexo as id,
								        			oda.descripcion as descr,
								        			nombre_documento,
													TO_CHAR(oda.fecha, 'DD-MM-YYYY') AS fecha_formateada
								        		from
								        			g_operadores.documentos_anexos oda
								        			, g_catalogos.documentos_anexos cda
								        		where " . $filtro . "
	...
	public function validarCodigoCatastral($conexion,$arrayParametros)
	{
	    $consulta = "SELECT COUNT(codigo_catastral) codigo_catastral_count
						FROM g_operadores.sitios
						WHERE codigo_catastral = '" . $arrayParametros['codigo_catastral'] ."'
						AND identificador_operador = '" . $arrayParametros['identificador_operador'] ."'
						GROUP BY codigo_catastral 
						HAVING COUNT(codigo_catastral) > 0;";

	    return $conexion->ejecutarConsulta($consulta);
	}


	public function obtenerRutaCertificado ($conexion, $identificadorOperador, $idTipoOperacion){
		...
		$res = $conexion->ejecutarConsulta("SELECT
										...
										WHERE
											...
											and op.identificador_operador = '$identificadorOperador'
											and scpoa.id_tipo_operacion in ($idTipoOperacion) 
											and scpoa.ruta_poa is not null 
											and scpoa.ruta_poa <> ''
											LIMIT 1;");
	...
	public function obtenerProductosOrganicosPorIdOperadorTipoOperacionHistoricoAsignados($conexion, $idOperadorTipoOperacion, $idHistorialOperacion, $estado){
			...
			$res = $conexion->ejecutarConsulta("SELECT 
													...
													dma.superficie_sitio,
													mias.codigo_magap,
													agce.nombre_agencia_certificadora
												FROM 
													g_operadores.operaciones o 
												INNER JOIN g_catalogos.productos p ON o.id_producto = p.id_producto 
												INNER JOIN g_catalogos.subtipo_productos sp ON p.id_subtipo_producto = sp.id_subtipo_producto 
												...
												INNER JOIN g_operadores.detalle_miembros_asociacion dma ON dma.id_operacion = o.id_operacion
												INNER JOIN g_operadores.miembros_asociacion mias ON mias.id_miembro_asociacion = dma.id_miembro_asociacion
												LEFT JOIN g_operadores.operaciones_organico oo ON oo.id_operacion = o.id_operacion
												INNER JOIN g_catalogos.agencia_certificadora agce ON agce.id_agencia_certificadora = oo.id_agencia_certificadora
												WHERE 
													o.id_operador_tipo_operacion = $idOperadorTipoOperacion 
													and o.id_historial_operacion = $idHistorialOperacion 
													and o.estado = '$estado'
	...
	public function obtenerProductosAreaSitioPorIdOperadorTipoOperacionHistoricoAsignados($conexion, $idOperadorTipoOperacion, $idHistorialOperacion, $estado){
