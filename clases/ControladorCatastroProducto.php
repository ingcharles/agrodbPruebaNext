...
class ControladorCatastroProducto {
	...
	public function obtenerTipoUsuario($conexion,$usuario){
		$res = $conexion->ejecutarConsulta("SELECT
											...
											WHERE
												...
												p.codificacion_perfil in ('PFL_USUAR_INT','PFL_USUAR_EXT', 'PFL_USUAR_CIV_PR', 'PFL_USUA_CONS_PO') and
	...
	public function obtenerCatastroPorNumeroLotePorIdAreaPorIdProducto($conexion, $idArea, $numeroLote, $idProducto, $cantidad) {
		...
		/*$consulta = "SELECT
						DISTINCT tc.id_catastro, tc.identificador_producto
						FROM
						(SELECT
							COALESCE (dc.identificador_producto, dc.identificador_unico_producto) as identificador_producto
						 , c.id_catastro
						...
						FROM 
							g_catastro.catastros c 
						INNER JOIN g_catastro.detalle_catastros dc ON c.id_catastro = dc.id_catastro) tc
						LEFT JOIN(SELECT 
							di.identificador, dv.id_tipo_operacion, dv.unidad_comercial, v.id_especie, v.estado
						FROM 
							g_vacunacion.vacunacion v
						INNER JOIN g_vacunacion.detalle_vacunacion dv ON v.id_vacunacion = dv.id_vacunacion
						INNER JOIN g_vacunacion.detalle_identificadores di ON di.id_detalle_vacunacion = dv.id_detalle_vacunacion 
						) tv ON tv.identificador = tc.identificador_producto
						WHERE
						tc.id_area = $idArea
						and tc.numero_lote = '$numeroLote'
						and tc.id_producto = $idProducto
						and tc.estado_registro = 'activo'
						and tv.id_tipo_operacion = tc.id_tipo_operacion 
						and tv.unidad_comercial = tc.unidad_comercial 
						and tv.id_especie = tc.id_especie
						and tv.estado NOT IN ('vigente')
						LIMIT $cantidad;";*/
			$consulta = "SELECT
						DISTINCT tc.id_catastro, tc.identificador_producto--, 'mejorada'::text
						FROM
						(SELECT
							COALESCE (dc.identificador_producto, dc.identificador_unico_producto) as identificador_producto
						 , c.id_catastro
						...
						INNER JOIN g_vacunacion.detalle_vacunacion dv ON v.id_vacunacion = dv.id_vacunacion
						INNER JOIN g_vacunacion.detalle_identificadores di ON di.id_detalle_vacunacion = dv.id_detalle_vacunacion						
						) tv ON tv.identificador = tc.identificador_producto and tv.id_tipo_operacion = tc.id_tipo_operacion 
						and tv.unidad_comercial = tc.unidad_comercial 
						and tv.id_especie = tc.id_especie
						and tv.estado NOT IN ('vigente')
						WHERE
	    ...
	    $res = $conexion->ejecutarConsulta ("SELECT
											...
											WHERE
												...
												and pa.codigo in ('PORHON','POROTE','PORACO','PORDRE','PORATE','PORONA','EQUTRO','EQULLO','EQUTRA','EQUGUA',
														  ...
														  'GRLABR','GRLPAR','GRLGAR','GRLOBR','GRLPOR','GRLGOR')
												--and ( ((select current_date - (to_date(to_char(c.fecha_nacimiento,'dd-mm-yyyy'),'dd-mm-yyyy')) )=ea.dias_cambio_etapa and pa.id_producto=ea.id_producto) or (ea.id_producto_etapa=c.id_producto and ea.estado_etapa='etapaFinal'))
												and dc.identificador_unico_producto in ('XXXXXXX')
											GROUP BY
												c.id_area
												,c.identificador_responsable
												,c.cantidad
